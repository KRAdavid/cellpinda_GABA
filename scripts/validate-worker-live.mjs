...
if (research.length !== 7) throw new Error(`Worker public API returned ${research.length} master studies; expected 7`);
...
if (!content.claims.find(claim => claim.id === 'research-sakashita-2019')?.metadata?.consumerContext) throw new Error('Worker public API dropped the Sakashita interpretation context');
if (!content.claims.find(claim => claim.id === 'research-powers-2008')?.metadata?.consumerDetail) throw new Error('Worker public API dropped the expanded Powers research detail');

console.log(JSON.stringify({origin:base.origin,staticPage:'ok',health:'cloudflare-d1',research:research.length,products:content.products.length,securityHeaders:'ok',status:'ok'}));
