...
const consumerMetadataKeys=['studyType','population','sampleSize','dose','duration','comparison','outcome','productApplicability','question','searchThrough','studyCount','consumerScope','consumerSummary','consumerFinding','consumerFindingFirst','consumerDetail','consumerContext','consumerDisclosure','consumerDisclosureStatus','consumerVisual','hopefulTakeaway'];
...
const products=ledger.products.filter(p=>p.status==='approved'&&p.sourceIds?.every(id=>ids.has(id))).map(p=>pick(p,['id','name','servings','category','officialUrl','status','availability','priceDisplay','sourceIds']));
const reviews=ledger.reviews.filter(r=>['shop-review-destination','shop-review-destination-1500'].includes(r.id)&&r.status==='approved').map(r=>pick(r,['id','status','publicText','sourceTitle','sourceUrl','originalPublic']));
