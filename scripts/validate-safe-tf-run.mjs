...
console.log(JSON.stringify({validatedAt: new Date().toISOString(), goalId: run.goalId, snapshotHash: run.snapshotHash, status: 'ok', preparation: run.preparation.id, checks: run.executed.map(item => item.id), humanGateTaskIds: run.humanGateTaskIds, candidateTaskIds: run.candidateTaskIds}));
