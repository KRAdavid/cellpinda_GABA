...
if (!/<noscript[\s>]/i.test(indexHtml) || !/GABA, 왜 중요한 성분일까요\?/.test(indexHtml) || !/사업자가 GABA의 역할과 중요성을 쉽게 설명하고 공유할 수 있도록 정리한 공개 안내서입니다\./.test(indexHtml) || !/수면·긴장·집중·운동부터 피부·인지·성장·면역까지/.test(indexHtml) || !/사업자가 바로 설명할 수 있는 GABA 5문장/.test(indexHtml) || /gaba-master-index\.json/i.test(indexHtml) || !indexHtml.includes(approvedSmartStoreUrl) || !indexHtml.includes(approvedSmartStoreReviewUrl) || !/<a href="https:\/\/smartstore\.naver\.com\/cellpinda\/products\/4701017202#REVIEW_DIALOG"[^>]*>스마트스토어에서 후기 읽기/.test(indexHtml)) fail('index.html must keep a readable static public GABA guide fallback and Smart Store product and review links');
...
if (!researchHtml.includes('운동 경험이 있는 남성 11명이 GABA 3g을 한 번 먹고, 운동 없이 쉰 조건과 운동 조건에서 90분 동안 혈액 속 수치를 살펴봤어요. 성장이나 근육 발달 효과를 확인한 연구는 아니며, 연구에 사용한 3g은 셀핀다 제품 섭취량의 근거가 아니에요.') || researchHtml.includes('GABA와 단백질을 함께 사용한 연구')) fail('research static fallback must match the approved Powers study scope and separate its research amount from the product');
