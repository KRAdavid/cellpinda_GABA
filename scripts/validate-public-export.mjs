...
if (!/<noscript[\s>]/i.test(indexHtml) || !/수면에서 인지, 피부, 근육과 성장 연구까지[\s\S]*이제 GABA를 알아야 합니다/.test(indexHtml) || !/잠들고, 집중하고, 움직이는 순간마다[\s\S]*GABA입니다\./.test(indexHtml) || !/3분 만에 GABA 이해하기[\s\S]*전문가 영상 보기[\s\S]*주제별 연구 보기/.test(indexHtml) || /gaba-master-index\.json/i.test(indexHtml) || /https:\/\/smartstore\.naver\.com\/cellpinda\/products\/4701017202|REVIEW_DIALOG|스마트스토어/.test(indexHtml)) fail('index.html must keep a readable static consumer GABA story without product or review CTAs');
...
if (!/<script type="application\/ld\+json">\{"@context":"https:\/\/schema\.org","@type":"WebSite","name":"GABA Guide · 뇌와 우리","url":"https:\/\/kradavid\.github\.io\/cellpinda_GABA\/"[^<]*"inLanguage":"ko-KR"\}<\/script>/.test(indexHtml)) fail('index.html must expose safe WebSite structured data');
