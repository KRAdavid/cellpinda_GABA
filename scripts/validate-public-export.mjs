...
if (!researchHtml.includes('연구용 양 GABA 3g을 한 번 먹고 혈액 속 성장호르몬을 본 연구예요. 셀핀다 가바 1500과는 별도 연구예요.') || researchHtml.includes('GABA와 단백질을 함께 사용한 연구')) fail('research static fallback must match the approved Powers study scope and separate its research amount from the product');
...
if (powers?.metadata?.consumerFindingFirst === true || !powers.metadata?.consumerSummary?.includes('남성 11명') || !powers.metadata.consumerSummary.includes('GABA 3g') || !powers.metadata.consumerSummary.includes('90분') || !powers.metadata.consumerFinding?.includes('성장호르몬 최고 수치') || !powers.metadata.consumerFinding.includes('운동 뒤 혈액 속 변화를 살펴본 자료') || !powers.metadata.consumerHighlight?.includes('운동 없이 쉰 조건') || /750mg 캡슐|750\s*제품|근육 크기와 근력 변화는 측정하지 않았어요/.test(JSON.stringify(powers.metadata)) || !powers.metadata.consumerDetail?.includes('약 4배')) fail('Powers study must lead with its measurement design, distinguish the resting condition, avoid removed SKU wording, and keep the numeric finding in context');
