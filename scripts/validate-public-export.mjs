...
const focusCardPng = await readFile(new URL('../public/assets/focus-game-card-v4.png', import.meta.url));
...
if (!researchComponent.includes('셀핀다 가바 1500 시험은 아니며, 연구마다 먹은 양과 살펴본 항목이 달라요.') || !researchHtml.includes('먹는 GABA 연구와 뇌 속 GABA 관찰 연구') || !researchHtml.includes('셀핀다 가바 1500 시험과는 구분해 보여드려요.')) fail('the research list must clearly distinguish general GABA studies, brain-GABA observation, and Cellpinda product information');
...
if(!stressSleepReview?.metadata?.consumerSummary?.includes('14편') || !stressSleepReview.metadata.consumerSummary.includes('대상과 섭취 조건') || !stressSleepReview.metadata.consumerSummary.includes('측정 항목') || stressSleepReview.metadata.consumerFinding !== undefined) fail('stress and sleep review must show its scope without surfacing an efficacy conclusion');
