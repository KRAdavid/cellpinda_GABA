...
const guideHtml = await readFile(new URL('../public/guide/index.html', import.meta.url), 'utf8');
...
if (!/<noscript[\s>]/i.test(indexHtml) || !/GABA는 뇌세포가 서로 신호를 주고받을 때 쓰이는 물질 가운데 하나예요/.test(indexHtml) || !/사람 연구에서 관찰한 내용을 쉽게 정리했어요\. 셀핀다 완제품 연구와는 다른 자료입니다\./.test(indexHtml) || !/셀핀다 가바 1500[^<]*30포/.test(indexHtml) || /가바 1,500\s*mg\s*[×x]\s*30포/i.test(indexHtml) || /gaba-master-index\.json/i.test(indexHtml) || !indexHtml.includes(approvedSmartStoreUrl) || !indexHtml.includes(approvedSmartStoreReviewUrl) || !/<a href="https:\/\/smartstore\.naver\.com\/cellpinda\/products\/4701017202#REVIEW_DIALOG"[^>]*>스마트스토어에서 후기 읽기/.test(indexHtml) || !indexHtml.includes(approvedReviewText)) fail('index.html must keep a readable static fallback with the approved research boundary and Smart Store product and review links');
if (!/사람 연구를 쉽게 보기/.test(indexHtml) || !/<strong>머리를 많이 쓴 뒤<\/strong> 성인 63명이 GABA 100mg과 비교 캡슐을 한 번씩 먹고, 뇌파와 활력 점수를 비교한 연구예요\./.test(indexHtml) || !/<strong>잠<\/strong> 성인 10명이 하루 GABA 100mg 캡슐과 비교 캡슐을 각각 1주 동안 먹고, 잠드는 시간과 수면 기록을 살펴본 연구예요\./.test(indexHtml) || !/href="\.\/research\/"/.test(indexHtml) || /성장호르몬|근육 발달|GABA 3g|운동 경험이 있는 남성|손끝 감각/.test(indexHtml)) fail('index.html no-script fallback must keep only the focused sleep and mental-task research summary');
if (!/<link rel="canonical" href="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/"\s*\/>/i.test(indexHtml) || !/<meta property="og:type" content="website"\s*\/>/i.test(indexHtml) || !/<meta property="og:url" content="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/"\s*\/>/i.test(indexHtml) || !/<meta property="og:image:type" content="image\/png"\s*\/>/i.test(indexHtml) || !/<meta property="og:image:width" content="1200"\s*\/>/i.test(indexHtml) || !/<meta property="og:image:height" content="630"\s*\/>/i.test(indexHtml)) fail('index.html must expose canonical metadata and a correctly sized Open Graph image');
...
if (!/canonical" href="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/focus\//.test(focusHtml) || !/property="og:title" content="“너도 해봐” 뇌컨디션 확인 챌린지"/.test(focusHtml) || !/property="og:image" content="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/assets\/focus-game-card-v5\.png"/.test(focusHtml) || !/focus=1#focus-game|focus=1/.test(focusHtml) || !focusHtml.includes('24개') || !focusHtml.includes('먼저 연습하고 시작하기') || !focusHtml.includes('뇌컨디션 확인 챌린지') || !focusCardSvg.includes('1분 색 신호 게임')) fail('focus invite page must expose its direct challenge title and randomized, user-started game preview');
...
if (!/canonical" href="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/research\//.test(researchHtml) || !/property="og:title" content="사람을 대상으로 한 GABA 연구를 쉽게 보기"/.test(researchHtml) || !/<meta property="og:image:type" content="image\/png">/.test(researchHtml) || !/<meta property="og:image:width" content="1200">/.test(researchHtml) || !/<meta property="og:image:height" content="630">/.test(researchHtml) || !researchHtml.includes('view=research') || /http-equiv="refresh"/.test(researchHtml) || researchHtml.includes(approvedSmartStoreUrl) || /href="#products"|셀핀다 제품 구성 확인/.test(researchComponent) || !researchComponent.includes("new URL('research/',base)") || !researchHtml.includes('가바 1500 제품 구성 보기') || !researchHtml.includes('view=products')) fail('research must have an independent public entry, stable deep links, a correctly sized social image, a neutral product-information handoff, and no direct product-purchase CTA');
if ((researchHtml.match(/일반 GABA 연구를 쉬운 말로 정리했어요\. 연구에서 먹은 양과 조건은 셀핀다 제품 표시와 다를 수 있어요\./g) ?? []).length < 4 || researchHtml.includes('잠·긴장·생각 과제에서 관찰한 내용을 그림으로 소개합니다. 셀핀다 완제품 연구와는 다른 자료입니다.')) fail('research static metadata and visible fallback must use the same product boundary copy');
if (!/<link rel="canonical" href="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/guide\/">/.test(guideHtml) || !/<meta property="og:url" content="https:\/\/kradavid\.github\.io\/cellpinda_GABA\/guide\/">/.test(guideHtml) || !guideHtml.includes('GABA, 우리 몸에서는 어떤 일을 할까요?') || !guideHtml.includes('../?view=guide') || !/application\/ld\+json[\s\S]*"inLanguage":"ko-KR"/.test(guideHtml)) fail('guide static route must expose guide-specific metadata and a readable handoff');
...
if (productSchema?.['@context'] !== 'https://schema.org' || !productSchemaNode || productSchemaNode.name !== '셀핀다 가바 1500' || 'category' in productSchemaNode || productSchemaNode.image !== 'https://kradavid.github.io/cellpinda_GABA/assets/product-composition-1500.png' || productSchemaNode.sameAs !== approvedSmartStoreUrl || productSchemaNode.offers || productSchemaNode.aggregateRating || productSchemaNode.review) fail('product share page must expose safe Product structured data without unverified classification, price, rating or review claims');
if (!/^User-agent: \*\nAllow: \/\nDisallow: \/cellpinda_GABA\/admin\nDisallow: \/cellpinda_GABA\/ops\nDisallow: \/cellpinda_GABA\/account\nDisallow: \/cellpinda_GABA\/\?view=account\nDisallow: \/cellpinda_GABA\/\?view=admin\nDisallow: \/cellpinda_GABA\/\?view=ops\n\nSitemap: https:\/\/kradavid\.github\.io\/cellpinda_GABA\/sitemap\.xml\s*$/m.test(robots)) fail('robots.txt must expose the public sitemap and keep internal paths out of discovery');
const sitemapUrls = [...sitemap.matchAll(/<loc>([^<]+)<\/loc>/g)].map(match => match[1]);
const expectedSitemapUrls = ['https://kradavid.github.io/cellpinda_GABA/', 'https://kradavid.github.io/cellpinda_GABA/products/', 'https://kradavid.github.io/cellpinda_GABA/research/', 'https://kradavid.github.io/cellpinda_GABA/guide/', 'https://kradavid.github.io/cellpinda_GABA/focus/', ...['active', 'sleep', 'irregular', 'sensory', 'unrested', 'steady'].map(id => `https://kradavid.github.io/cellpinda_GABA/share/${id}/`)];
...
if (!researchComponent.includes("const publicConsumerResearchIds = new Set(['research-yamatsu-2016', 'research-yoto-2012'])") || !researchComponent.includes('publicConsumerResearchIds.has(claim.id)')) fail('the public research library must allow only the focused sleep and mental-task studies while retaining the full master index for audit');
