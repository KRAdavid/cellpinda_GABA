...
if (powers?.metadata?.consumerFindingFirst === true || !powers.metadata?.consumerSummary?.includes('남성 11명') || !powers.metadata.consumerSummary.includes('GABA 3g') || !powers.metadata.consumerSummary.includes('90분') || !powers.metadata.consumerFinding?.includes('성장호르몬 최고 수치') || !powers.metadata.consumerFinding.includes('운동 뒤 혈액 속 변화를 살펴본 자료') || /750mg 캡슐|750\s*제품|근육 크기와 근력 변화는 측정하지 않았어요/.test(JSON.stringify(powers.metadata)) || !powers.metadata.consumerDetail?.includes('약 4배')) fail('Powers study must lead with its measurement design, avoid removed SKU wording, and keep the numeric finding in context');
