...
if (!powers?.metadata?.consumerSummary?.includes('성장호르몬 수치') || !powers.metadata?.dose?.includes('750mg 캡슐 4개') || !powers.metadata?.consumerScope?.includes('설탕 캡슐') || !powers.metadata?.consumerFinding?.includes('약 400%') || !powers.metadata.consumerFinding.includes('비교 조건') || /음료|마시|근육이 커|근력이 좋아/.test(powersConsumerData) || powers.metadata?.consumerVisual || /4배|높았/.test(powers.publicText)) fail('Powers study must accurately state the measured hormone result, dose and study condition without implying muscle growth');
