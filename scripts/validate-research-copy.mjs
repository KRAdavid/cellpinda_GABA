...
if (!consumerUi.includes('셀핀다 완제품을 시험한 결과는 아닙니다') || !researchLibrary.includes('연구마다 사용한 제품·양·기간이 달라요') || !researchLibrary.includes('GABA를 먹지 않고 뇌 신호를 살펴본 연구') || !researchLibrary.includes('연구에서 기록한 내용') || !researchLibrary.includes('metadata.productApplicability') || !researchLibrary.includes("new URL('research/',base)") || !researchLibrary.includes('논문 원문 보기') || !researchLibrary.includes('source.title')) fail('research route must explain product scope, intake versus observation, study context and sources in consumer language');
