...
if (!review?.metadata?.consumerSummary?.includes('14편') || !review.metadata.consumerSummary.includes('대상과 섭취 조건') || !review.metadata.consumerSummary.includes('측정 항목') || review.metadata.consumerFinding !== undefined) fail('2020 review must stay focused on the review scope and must not surface its discouraging efficacy summary');
...
if (!yamatsu?.publicText?.includes('수면 설문에서 잠의 질이 낮게 나온 일본 성인 10명') || !yamatsu.publicText.includes('2.2% 높게') || /2\.2%포인트|2\.2%p/.test(yamatsuEvidence) || !yamatsu.metadata?.consumerScope?.includes('수면 설문에서 잠의 질이 낮게 나온') || yamatsu.metadata.consumerScope.includes('100mg') || yamatsu.metadata.consumerScope.includes('1주') || !yamatsuVisual?.metrics?.some(metric => metric.label.includes('빠른 눈 움직임이 없는 수면 단계 비율') && metric.value === '2.2' && metric.unit === '%' && metric.trendLabel?.includes('비교 조건보다 높게'))) fail('Yamatsu must state the selected sleep-quality population, use the reported 2.2% unit consistently and avoid repeating dose and duration outside its visual');
