...
if (!review?.metadata?.consumerFinding?.includes('2020년 2월까지') || !review.metadata.consumerFinding.includes('14편') || !review.metadata.consumerFinding.includes('긴장 관련') || !review.metadata.consumerFinding.includes('수면은 결론을 내리기엔 자료가 적었어요') || !review.metadata.consumerFinding.includes('연구마다 결과가 달랐고')) fail('2020 review must state the review authors’ stress and sleep assessment in plain language');
...
if (!powers?.metadata?.consumerSummary?.includes('성장호르몬 수치') || !powers.metadata?.dose?.includes('750mg 캡슐 4개') || !powers.metadata?.consumerScope?.includes('설탕 캡슐') || !powers.metadata?.consumerDetail?.includes('쉬는 조건') || !powers.metadata.consumerDetail.includes('운동 조건') || !powers.metadata.consumerDetail.includes('근육 크기나 근력은 측정하지 않았어요') || /약 4배|400%/.test(powers.metadata.consumerDetail) || /음료|마시|근육이 커|근력이 좋아/.test(powersConsumerData) || powers.metadata?.consumerVisual || /4배|높았/.test(powers.publicText)) fail('Powers study must show both test conditions and the boundary on what was measured without ambiguous fold claims');
const sakashita = research.find(claim => claim.id === 'research-sakashita-2019');
if (!sakashita?.metadata?.consumerFinding?.includes('1.34kg') || !sakashita.metadata.consumerFinding.includes('0.15kg') || !sakashita.metadata.consumerContext?.includes('그룹 간 효과는 확정적으로 해석하기 어려워요') || sakashita.metadata.consumerVisual) fail('Sakashita study must show source-reported averages, flag the analysis conflict and suppress the disputed comparison graphic');
...
if (!yamatsuVisual?.metrics?.some(metric => metric.label.includes('눈이 빠르게 움직이지 않는 수면 비율') && metric.value === '2.2' && metric.unit === '%포인트' && metric.trendLabel?.includes('비교 조건보다 높게'))) fail('Yamatsu must label the 2.2 result as percentage points of total sleep time in plain language');
