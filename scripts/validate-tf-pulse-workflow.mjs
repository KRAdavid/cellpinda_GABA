...
if (/gh workflow run deploy\.yml/.test(source)) issues.push('heartbeat workflow에서 deploy workflow를 직접 dispatch하면 안 됩니다. PR merge 후 main 배포가 실행됩니다.');
...
if (!(commitIndex >= 0 && commitIndex < pushIndex && pushIndex < prListIndex && prListIndex < prCreateIndex)) issues.push('heartbeat 커밋·브랜치 push·기존 PR 검사·PR 생성 순서가 올바르지 않습니다.');
...
if (issues.length) {
  ...
  console.log(JSON.stringify({workflow: '.github/workflows/tf-pulse.yml', status: 'ok', schedule: '17 */6 * * *', persistence: 'protected-main PR', reviewGate: 'required checks + human merge'}));
}
