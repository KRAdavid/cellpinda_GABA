...
for (let attempt = 1; attempt <= 12; attempt += 1) {
  try {
    ...
    assert.ok(consumerBundle.includes('색 신호에 맞춰 누르거나 멈추는 1분 게임이에요. 점수로 뇌 피로나 건강 상태를 판단하지 않아요.') && consumerBundle.includes('초록') && consumerBundle.includes('보라'), 'live consumer bundle must clarify the non-diagnostic game and show accessible color labels');
