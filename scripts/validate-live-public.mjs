...
for (let attempt = 1; attempt <= 12; attempt += 1) {
  try {
    ...
    assert.ok(yoto2012?.metadata?.consumerFinding?.includes('뇌파와 활력 설문 점수의 감소 폭') && yoto2012.metadata.consumerFinding.includes('비교 조건보다 작게 기록됐어요') && !/안정적으로 유지|뇌 활력 개선|개선됐/.test(yoto2012.metadata.consumerFinding) && yoto2012.metadata.consumerVisual?.outcomes?.some(item => item.label === '활력 설문 점수' && item.result.includes('감소 폭이 작게 기록됐어요')), 'live Yoto copy must describe the observed decrease range against the comparison condition without implying a general stabilizing or improvement effect');
