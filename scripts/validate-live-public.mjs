...
for (let attempt = 1; attempt <= 12; attempt += 1) {
  try {
    ...
    assert.ok(researchPageText.includes('연구용 양 GABA 3g을 한 번 먹고 혈액 속 성장호르몬을 본 연구예요. 셀핀다 가바 1500과는 별도 연구예요.') && !researchPageText.includes('GABA와 단백질을 함께 사용한 연구'), 'live research fallback must match the approved Powers study scope and separate its research amount from the product');
    ...
    assert.ok(consumerBundle.includes('잠드는 시간이 비교 캡슐을 먹은 주보다 평균 5분 짧게 기록됐어요.') && consumerBundle.includes('과제 뒤 뇌파와 활력 점수가 비교 캡슐을 먹었을 때보다 덜 떨어졌어요.') && consumerBundle.includes('연구에서 관찰된 내용') && consumerBundle.includes('연구용 양: 하루 100mg') && consumerBundle.includes('연구용 양: 100mg 1회') && consumerBundle.includes('제품 표기 섭취량과 달라요'), 'live consumer bundle must preserve plain-language research highlights and separate research amounts from product servings');
