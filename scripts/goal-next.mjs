...
const verifying = graph.tasks.filter(task => task.state === 'VERIFYING').sort((a, b) => a.priority - b.priority || a.id.localeCompare(b.id));
const summary = { goalId: graph.goalId, goalStatus: contract.status, ready, verifying, waiting, counts: Object.fromEntries(graph.stateMachine.map(state => [state, graph.tasks.filter(task => task.state === state).length])) };
if (process.argv.includes('--check')) {
  console.log(JSON.stringify({ goalId: summary.goalId, ready: ready.length, verifying: verifying.length, waiting: waiting.length, tasks: graph.tasks.length }));
} else {
  ...
  console.log(`독립 검증 진행 ${verifying.length}개`);
  for (const task of verifying) console.log(`- ${task.id} [VERIFYING] ${task.title} · 담당 ${task.lead} · 검증 ${task.verifier} · 다음: 검증 증거와 수락 기준을 대조해 DONE 또는 REWORK 판정`);
