...
requireMatch(app, /const focusInvite=[\s\S]*const routeUrl=focusInvite[\s\S]*share\/\$\{type!\.id\}\/[\s\S]*update\('og:url','property',routeUrl\)[\s\S]*canonical\.href=routeUrl[\s\S]*updateStructuredData\(\{'@context':'https:\/\/schema\.org','@type':'WebPage'/, 'focus and shared-result query views must keep canonical, Open Graph and JSON-LD metadata on their static deep-link routes');
...
requireMatch(rhythm, /rhythm-result-quick-action[\s\S]*?뇌컨디션 확인 챌린지 해보기[\s\S]*?rhythm-result-commerce[\s\S]*?rhythm-result-invite[\s\S]*?친구에게 1분 체크 보내기[\s\S]*?내 답변과 점수는 전송되지 않아요[\s\S]*?rhythm-more-share[\s\S]*?결과 카드 공유·저장 등 다른 방법/, 'result actions must guide the visitor from the immediate result action to product, invitation and secondary sharing');
