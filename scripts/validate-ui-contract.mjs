...
for (const marker of ['잠드는 시간과 수면 기록', '활력 점수는 덜 떨어지고, 뇌파 변화 폭은 작았어요', '쉬었을 때와 운동했을 때의 혈액 속 변화', '그림으로 한눈에 보기', '전체 연구 카드 보기']) requireMatch(gabaResearchHighlights, new RegExp(marker), `post-teaser GABA research highlight ${marker} is missing`);
...
requireMatch(indexHtml, /먼저 확인해 주세요\.[\s\S]*연구에서 먹은 양은 셀핀다 제품에 적힌 양과 달라요\.[\s\S]*<strong>머리를 많이 쓴 뒤<\/strong> 성인 63명이 GABA 100mg을 한 번 먹은 뒤, 활력 점수는 덜 떨어지고 뇌파 변화 폭은 작게 기록된 연구예요\.[\s\S]*<strong>수면<\/strong> 성인 10명이 하루 100mg을 먹은 주와 비교 캡슐을 먹은 주의 잠드는 시간과 수면 기록을 살펴본 연구예요\./, 'no-script research summary must use the current consumer topics, study amounts and product boundary');
