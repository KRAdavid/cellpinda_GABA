...
export function createStore(db:D1Database) {
  ...
  return {
    ...
    async update(id:string,patch:unknown) {
      ...
      if(row.kind==='review' && before.id==='shop-review-destination-1500' && typeof patch.publicText==='string' && patch.publicText.trim()!==REVIEW_DESTINATION_TEXT)throw failure('Review destination text is fixed; submit quotations through the review workflow');
    ...
    async analytics() {
      ...
      return {counts:counts.results,byProduct:byProduct.results,funnels,sharingMetrics,window:{kind:'all_collected_events',from:range?.value ?? null,to:new Date().toISOString(),ordering:'Server receipt order; no client timestamps',flowScope:'Browser in-memory page lifetime, not unique people'},coverage,actualPurchases:{supported:false,count:null,reason:'No verified order integration. Purchase clicks are not purchases.'},returningVisitors:{supported:false,rate:null,reason:'Seven-day return requires a future consented identity policy.'},sharing:{confirmedDeliverySupported:false,requestToLandingRate:{supported:false,rate:null,reason:'Sender and recipient are different flows; no matched referral attribution.'}},auditCount:audit?.count ?? 0};
    },
  };
}
