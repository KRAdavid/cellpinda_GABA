...
const PUBLIC_META=new Set(['studyType','population','sampleSize','dose','duration','comparison','outcome','productApplicability','question','searchThrough','studyCount','consumerScope','consumerSummary','consumerFinding','consumerHighlight','consumerFindingFirst','consumerVisual','consumerDetail','consumerContext','consumerDisclosure','hopefulTakeaway']);
...
const CONTROLLED_PRODUCT_FIELDS=['name','amountMg','servings','totalG','category','officialUrl','availability','sourceIds'] as const;
...
export function createStore(db:D1Database) {
  ...
  return {
    ...
    async publicContent() {
      ...
      const products=items.filter(p=>p.kind==='product' && p.status==='approved' && p.sourceIds?.length && p.sourceIds.every(id=>ids.has(id))).map(p=>Object.fromEntries(['id','name','amountMg','servings','totalG','category','officialUrl','availability','priceDisplay','sourceIds','revision','publicText'].filter(key=>p[key]!==undefined).map(key=>[key,p[key]])));
