...
export function socialMetadata(requestUrl:string,configuredOrigin?:string) {
  ...
  const account=url.pathname==='/account' || url.pathname.startsWith('/account/');
  ...
  const title=account?'내 리듬 기록 | Cellpinda':admin?'콘텐츠 검토실 | Cellpinda':productView?'셀핀다 가바 제품 구성 비교 | Cellpinda':type?`공유받은 하루 리듬: ‘${resultTypes[type].shareLabel} · ${resultTypes[type].name}’ | Cellpinda`:'오늘, 내 뇌는 쉴 틈이 있었을까? | Cellpinda';
  const description=account?'내가 저장한 하루 리듬 기록을 확인합니다.':admin?'셀핀다 콘텐츠 운영자를 위한 검토실입니다.':productView?'1포 내용량과 구성을 확인하고 스마트스토어에서 구매 조건을 살펴보세요.':type?`공유받은 ‘${resultTypes[type].shareLabel} · ${resultTypes[type].name}’의 이야기를 살펴보세요. 링크를 연 사람의 결과가 아니며, 의학적 진단이나 체내 GABA 측정이 아닙니다.`:'나의 하루 리듬을 돌아보고, GABA 이야기와 셀핀다 발효 가바 제품을 알아보세요.';
  const canonical=productView ? `${origin}/products/` : `${origin}/${type?`?rhythm=${type}`:''}`;
  ...
  return {title,description,canonical,image,imageAlt,robots:admin?'noindex, nofollow, noarchive':'index, follow',admin,type,view:productView?'products':null};
}
...
export function socialTags(meta:ReturnType<typeof socialMetadata>) {
  ...
  return [tag('description',meta.description),tag('robots',meta.robots),tag('og:type',meta.view==='products'?'product':'website',true),tag('og:site_name','Cellpinda',true),tag('og:locale','ko_KR',true),tag('og:title',meta.title,true),tag('og:description',meta.description,true),tag('og:url',meta.canonical,true),tag('og:image',meta.image,true),tag('og:image:type','image/png',true),tag('og:image:width','1200',true),tag('og:image:height','630',true),tag('og:image:alt',meta.imageAlt,true),tag('twitter:card','summary_large_image'),tag('twitter:title',meta.title),tag('twitter:description',meta.description),tag('twitter:image',meta.image),tag('twitter:image:alt',meta.imageAlt),`<link rel="canonical" href="${escape(meta.canonical)}">`].join('');
}
