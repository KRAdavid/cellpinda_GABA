...
test('D1 approval transaction, seed preservation, public filtering and audit',async()=>{
  ...
  try {
    ...
    let data=await store.publicContent();assert.equal(data.claims.length,1);assert.equal(data.products.length,1);assert.ok(!JSON.stringify(data).includes('secret'));assert.ok(!JSON.stringify(data).includes('Internal observation'));assert.ok(!JSON.stringify(data).includes('Internal note'));
...
test('Worker and Node expose only the approved Smart Store 1500 review destination, never private quotes',async()=>{
  const reviewedSeed={...seed,reviews:[{id:'shop-review-destination-1500',status:'approved',originalPublic:true,publicText:'스마트스토어 1500 후기 보기',sourceTitle:'스마트스토어 1500',sourceUrl:'https://smartstore.naver.com/cellpinda/products/4701017202#REVIEW_DIALOG',limitations:['개인 경험'],holdReason:'internal',original:'PRIVATE ORIGINAL 1500'},{id:'private-review',status:'hold',publicText:'PRIVATE QUOTE',sourceUrl:null},{id:'unverified-review',status:'approved',publicText:'UNVERIFIED QUOTE',sourceUrl:'https://cellpinda.co.kr/'}]};
...
test('Persistent stores migrate an obsolete canonical review destination to Smart Store without touching quote reviews',async()=>{
  const canonical={id:'shop-review-destination-1500',status:'approved',originalPublic:true,publicText:'스마트스토어에서 가바 1500 구매자 후기와 다양한 사용 경험을 확인하세요.',sourceTitle:'셀핀다 스마트스토어 가바 1500 상품 후기',sourceUrl:'https://smartstore.naver.com/cellpinda/products/4701017202#REVIEW_DIALOG',reviewedAt:'2026-09-10',limitations:['구매자 후기는 개인 경험이며 의학적 효능을 보장하지 않습니다.']};
  ...
  try {
    ...
    await cloud.initialize(currentSeed);let output=await cloud.publicContent();assert.equal(output.reviews.length,1);assert.equal(output.reviews[0].sourceUrl,'https://smartstore.naver.com/cellpinda/products/4701017202#REVIEW_DIALOG');
    const first=createNodeStore({dbPath,seed:staleSeed});first.close();
    const local=createNodeStore({dbPath,seed:currentSeed});output=local.publicContent();assert.equal(output.reviews.length,1);assert.equal(output.reviews[0].sourceUrl,'https://smartstore.naver.com/cellpinda/products/4701017202#REVIEW_DIALOG');assert.equal(output.reviews[0].status,'approved');local.close();
