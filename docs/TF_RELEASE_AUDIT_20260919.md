...
후속 소비자 전환 감리에서 챌린지 완료 카드의 다음 행동이 친구 초대·재시작에만 머무는 점을 확인해 `GABA 이야기`·`가바 1500 구성`·승인된 스마트스토어 `구매자 후기`를 3단계 링크로 추가했다. 점수·답변·건강 상태를 공유하지 않으며, 각 링크는 기존 승인 목적지와 익명 이벤트 경계를 따른다. 후보 커밋 `23a0919`에서 정적 번들·UI 계약·전체 테스트·필수 PR 검사를 다시 통과했다.
