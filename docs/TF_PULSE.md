...
같은 workflow는 pulse 생성 직후 `run-safe-tf-actions.mjs`를 실행한다. 이 단계는 Goal Contract·연구 문구·티저 경계·샌드박스 MVP·공개 export·TF pulse 검증처럼 읽기 전용인 여섯 가지 내부 검사를 매회 재실행하고, 결과를 `safe_internal_tf_run` artifact와 run summary에 남긴다. 자동 실행 경계는 `A_READ`로 고정되어 외부 게시·구매·승인·canonical 업무 그래프 상태 변경을 하지 않으며, 사람 판단 게이트를 감시하는 동안 내부 품질 확인을 멈추지 않는다.
