...
이 문서의 아래 기록은 작업 당시의 후보·검토 로그를 보존한 것이다. 현재 공개 판정은 이 절을 기준으로 한다. 시각형 리엘 기능을 반영한 병합 커밋은 `74c8bb3e3f9545b17380dba8b6b1156dbed763fc`이며, [기능 배포 run 35581359157](https://github.com/KRAdavid/cellpinda_GABA/actions/runs/35581359157)가 `release-verify`·Pages·라이브 smoke·release status까지 성공했다. 공개 `release-manifest.json`은 HTTP 200으로 같은 SHA를 가리키며, 직접 라이브 검증도 `provenance: matched`로 통과했다. 결과 화면은 `내 상태 확인 → 뇌컨디션 확인 챌린지 → 제품·후기 → 친구 공유` 순서로 정리했고, 챌린지 시작 화면은 `먼저 연습하고 시작하기`를 주 행동으로 고정했다. 히어로 아래에는 4장 시각형 GABA 리엘을 배치해 리듬 체크 다음에 상태·휴식·일반 연구·1분 체크 흐름을 짧게 보여준다.

라이브 검증에서 정적 모드·공개 경로 10개·hash 파일 65개·승인 연구 6건·공개 주장 12개·제품 1개·후기 목적지 1개·공유 경로 6개를 확인했다. 구매·후기 목적지는 지정된 Smart Store 한 곳으로 고정했고 750 제품과 내부 운영 스냅샷은 공개본에서 제외했다. 모바일 연구 카드 줄바꿈과 뇌 근거 라벨 대비 P2를 보정했고, 390px CDP에서 리엘 카드 4장·가로 넘침 0·콘솔 오류 0·접근성 이름 누락 0을 재확인했다. 티저는 `PREVIEW` 상태로 경계가 표시되며, 최종 공개 권리·콘텐츠 승인은 아직 B4 `WAITING`이다.
