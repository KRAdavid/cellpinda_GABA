...
PR #125·#126·#127·#128·#129·#130·#132·#134·#135·#136은 필수 검사 통과 후 동일 계정의 마지막 푸시 승인 제한으로 관리자 병합을 진행했고, 병합 순간에만 보호 규칙을 일시 완화한 뒤 즉시 복구했다. 현재 보호 규칙은 Code Owner 리뷰 1명, 필수 검사 `release-verify`·`site-quality-verify`, 마지막 푸시 승인, 대화 해결, 선형 이력, 관리자 강제 적용을 모두 유지한다. 현재 열린 PR은 없다. 이 관리자 우회는 독립 Code Owner 승인을 대체한 운영 기록이며, 실제 독립 승인이 완료됐다고 해석하지 않는다. PR #132에서는 연구 결과의 비교 대상과 측정 항목을 소비자 문장으로 명확히 하고, 일반 GABA 연구 제목과 카드 넘김 안내를 보정했다. PR #134에서는 root·연구 fallback의 연구 조건을 동기화하고, 승인 전 티저를 공개본에서 차단했으며, 결과 뒤 리엘 CTA를 제품 구성으로 연결했다. PR #136에서는 `/products/` 직접 유입의 제품 섹션 안착을 보강했다.
