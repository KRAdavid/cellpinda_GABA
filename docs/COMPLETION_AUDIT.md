...
이 문서의 아래 기록은 작업 당시의 후보·검토 로그를 보존한 것이다. 현재 공개 판정은 이 절을 기준으로 한다. 최신 main 병합 커밋은 `9918f40e18194a6b01f8e1dd9e19b4807d3aa7b4`이며, [배포 run 35578802038](https://github.com/KRAdavid/cellpinda_GABA/actions/runs/35578802038)가 `release-verify`·Pages·라이브 smoke·release status까지 성공했다. 공개 `release-manifest.json`은 HTTP 200으로 같은 SHA를 가리키며, 직접 라이브 검증도 `provenance: matched`로 통과했다. 결과 화면은 `내 상태 확인 → 뇌컨디션 확인 챌린지 → 제품·후기 → 친구 공유` 순서로 정리했고, 챌린지 시작 화면은 `먼저 연습하고 시작하기`를 주 행동으로 고정했다.
...
PR #116·#117·#118·#120 관리자 병합은 필수 리뷰 조건만 병합 순간에 일시 완화하고 즉시 복구했다. 현재 보호 규칙은 Code Owner 리뷰 1명, 필수 검사 `release-verify`·`site-quality-verify`, 마지막 푸시 승인, 대화 해결, 선형 이력, 관리자 강제 적용을 모두 유지한다. GitHub Pages 환경도 `can_admins_bypass=false`로 설정해 관리자 우회를 차단했다. PR #120 병합 뒤에는 리뷰 보호 원복 응답과 전체 branch protection을 다시 조회해 확인했다. 현재 열린 PR은 없다.

## 2026-09-21 시각형 GABA 리엘 후보 — 4장으로 축소해 검증 중

텍스트가 길어지는 문제를 줄이기 위해 `ConsumerGabaReel`을 추가했다. 소비자 흐름을 `지금 내 상태 → 쉬어야 하는 이유 → 일반 GABA 연구에서 본 변화 → 1분 체크` 4장으로 구성하고, 연구·제품·후기는 각각 원래 화면에서 확인하도록 분리했다. 11장 후보는 390px에서 1분 체크 시작 위치를 약 955px 밀고 연구·제품·후기 문구를 반복해 보류했으며, 4장 후보는 리엘을 리듬 체크 뒤에 배치해 핵심 CTA를 먼저 보여준다. 모바일 CDP에서 390px 가로 넘침 0, 콘솔 오류 0, 접근성 이름 누락 0을 확인했고, `prefers-reduced-motion` 스크롤과 리엘 키보드 포커스 표시를 보강했다. 히어로의 `GABA 한 장씩 보기`는 컴포넌트가 준비된 뒤 확실히 이동하도록 지연 앵커 처리를 적용했다. 이 후보는 아직 PR·main·라이브에 승격하지 않았으며, 다음 단계는 필수 검사와 라이브 smoke를 통과한 뒤 Code Owner 검토를 받는 것이다.
