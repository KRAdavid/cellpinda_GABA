...
이 문서의 아래 기록은 작업 당시의 후보·검토 로그를 보존한 것이다. 공개 판정의 권위는 항상 [공개 release-manifest.json](https://kradavid.github.io/cellpinda_GABA/release-manifest.json)의 `candidateSha`, 생성 시각, 검사 결과와 [최신 성공 배포 run](https://github.com/KRAdavid/cellpinda_GABA/actions)이다. 이 문서는 감사 시점 스냅샷으로 현재 확인값을 기록한다: main `570a63efa7d3e044ceab7c199362bc3686ae3d7d`, 배포 run [35599044204](https://github.com/KRAdavid/cellpinda_GABA/actions/runs/35599044204), `release-verify`·Pages 게시·라이브 smoke·release status 성공. 문서만 갱신하는 병합에서도 manifest의 candidate SHA가 바뀔 수 있으므로, 아래 SHA를 영구적인 최신값으로 해석하지 않는다. 결과 화면은 `내 상태 확인 → 뇌컨디션 확인 챌린지 → 제품·후기 → 친구 공유` 순서로 정리했고, 1분 체크 전에는 챌린지를 숨겨 다음 행동을 하나로 고정했다. 직접 초대 경로에서는 챌린지를 바로 열 수 있다. 히어로 아래에는 4장 시각형 GABA 리엘을 배치해 리듬 체크 다음에 상태·휴식·일반 연구·제품 구성 흐름을 짧게 보여주며, `/products/` 직접 진입도 제품 섹션에 자동 안착한다.

라이브 검증에서 정적 모드·공개 경로 10개·hash 파일 65개·승인 연구 6건·공개 주장 12개·제품 1개·후기 목적지 1개·공유 경로 6개를 확인했다. 구매·후기 목적지는 지정된 Smart Store 한 곳으로 고정했고 750 제품과 내부 운영 스냅샷은 공개본에서 제외했다. 모바일 연구 카드 줄바꿈과 연구 측정 범위 문구를 보정했고, 최근 390px·1440px CDP 감리에서 P0/P1 결함·가로 넘침·콘솔 오류를 확인하지 않았다. 외부 티저는 B4 승인 전 `HOLD`로 전환해 공개 URL·iframe을 제거했고, 최종 공개 권리·콘텐츠 승인은 아직 B4 `WAITING`이다.

PR #125·#126·#127·#128·#129·#130·#132·#134·#135·#136·#137·#138·#139은 필수 검사 통과 후 동일 계정의 마지막 푸시 승인 제한으로 관리자 병합을 진행했고, 병합 순간에만 보호 규칙을 일시 완화한 뒤 즉시 복구했다. 현재 보호 규칙은 Code Owner 리뷰 1명, 필수 검사 `release-verify`·`site-quality-verify`, 마지막 푸시 승인, 대화 해결, 선형 이력, 관리자 강제 적용을 모두 유지한다. 현재 열린 PR은 없다. 이 관리자 우회는 독립 Code Owner 승인을 대체한 운영 기록이며, 실제 독립 승인이 완료됐다고 해석하지 않는다. PR #132에서는 연구 결과의 비교 대상과 측정 항목을 소비자 문장으로 명확히 하고, 일반 GABA 연구 제목과 카드 넘김 안내를 보정했다. PR #134에서는 root·연구 fallback의 연구 조건을 동기화하고, 승인 전 티저를 공개본에서 차단했으며, 결과 뒤 리엘 CTA를 제품 구성으로 연결했다. PR #136에서는 `/products/` 직접 유입의 제품 섹션 안착을 보강했고, PR #138에서는 제품 섹션 이동 CTA를 `가바 1500 제품 구성 보기`로 통일했다.
