...
이 문서의 아래 기록은 작업 당시의 후보·검토 로그를 보존한 것이다. 현재 공개 판정은 이 절을 기준으로 한다. 최신 애플리케이션 변경 커밋은 `c1fda83d2476b913102e2cc6d8aeca91ef102ceb`이며, [배포 run 35572028035](https://github.com/KRAdavid/cellpinda_GABA/actions/runs/35572028035)가 성공했다. 공개 `release-manifest.json`은 HTTP 200으로 현재 main 후보 `c1fda83d2476b913102e2cc6d8aeca91ef102ceb`를 가리킨다. 첫 화면 체크 CTA와 결과 화면의 제품·후기 CTA는 소비자가 다음 행동을 바로 이해하도록 정리했다.
...
## 2026-09-21 TF heartbeat 신선도 배포 게이트 보강 — 후보 작업 중

배포 검증에 `validate:tf-pulse-freshness`를 추가해 TF heartbeat가 없거나 8시간보다 오래되면 Pages 게시 전 `release-verify`가 실패하도록 했다. 신선한 heartbeat만 배포 후보를 통과시키며, 이 검사는 사람 승인 게이트(B2·B3·B4·C2·E1)를 자동으로 완료시키지 않는다. 신선·만료 경계 테스트와 전체 회귀 테스트를 통과했고, 보호된 main 병합 뒤 새 Pages run에서 다시 확인한다.
