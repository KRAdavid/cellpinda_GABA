...
사용자가 제공한 발효가바 티저 URL을 리듬 체크 다음의 선택형 카드에 연결했다. `public/data/teaser-preview.json`을 공개 데이터로 생성해 외부 페이지 제목·설명·주의 문구와 HTTPS 주소를 관리하고, 카드 노출·티저 페이지 열기 이벤트를 익명 경로로 기록한다. 소비자 흐름은 자기 점검→티저 선택→GABA 연구·제품 정보 확인 순서로 유지한다.
