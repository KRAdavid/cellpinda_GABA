...
전체 목표는 기술 공개 단계까지 진행됐지만 운영 완료로 승격하지 않는다. B2(최종 제품 표시), B3(후기 재게시 권한), B4(티저 공개 승인), C2(Worker·D1 운영과 비밀·복구 검증), E1(실구매 대사)은 사람 승인 또는 외부 운영 입력을 기다린다. PR #120에서 B2/B3/E1이 `DONE`으로 바뀔 때 독립 증빙 원장을 요구하는 검증을 배포 파이프라인에 연결했으며, 현재는 B2 `VERIFYING`, B3/E1 `WAITING`을 유지한다. Worker/D1 필수 Secret이 없는 환경에서는 정적 Pages만 공개하고 `STATIC_ONLY`로 기록한다. 이 경계를 유지하는 것이 완료 판정을 과장하지 않고 감사 가능성을 보존하는 기준이다.
...
티저를 `APPROVED`로 전환할 때 승인 체크리스트의 각 항목에 독립 검토자, 승인 시각, 근거 파일 SHA-256, 승인 범위를 함께 기록하도록 검증을 강화했다. 현재 티저는 B4 승인 전 `HOLD` 상태이며, 공개 iframe·미디어 URL로 자동 전환하지 않는다.
...
첫 화면 CTA 아래에 `1분 뒤, 내 상태와 오늘 해볼 일을 확인해 보세요.`를 추가해 체크의 기대 결과를 바로 알 수 있게 했다. 결과 화면의 구매 행동은 `가바 1500 구성·가격 확인하기`, 후기 행동은 `스마트스토어 구매자 후기 읽기`로 분리했다. `pnpm test`, 타입검사, 연구 문구·UI 계약·공개 export·정적 번들 검증과 라이브 smoke가 모두 통과했다. Pages 환경은 기존 main 브랜치 정책을 유지하면서 관리자 우회만 차단했다.

## 2026-09-21 라이브 smoke용 티저 자동재생 안내 보강 — 후보 작업 중

Pages 게시 뒤 라이브 smoke가 지연 로딩 청크의 티저 fallback 문구를 루트 소비자 번들에서 확인하지 못해 `RELEASE_FAILED`가 기록됐다. 루트 화면에 접근 가능한 자동재생·사용자 재생 안내를 추가하고 타입검사·UI 계약·110개 회귀 테스트·production build·성능 예산을 다시 통과시켰다. 보정 커밋은 보호 규칙을 복원한 뒤 별도 후보 PR로 재검증한다.
...
로컬 완제품 감사 매니페스트에 사용자가 처음 지정한 `사용자가 지정한 로컬 자료 폴더`을 요청 경로로 보존하고, 현재 PC에서 실제 발견된 `사용자가 지정한 로컬 자료 폴더`을 함께 탐색하도록 했다. 감사는 같은 파일을 중복 계산하지 않으며, 자료가 없는 경로는 다른 루트의 후보 판정에 영향을 주지 않는다. 최신 로컬 사이클은 완제품 후보 5건·원료 벌크 자료 제외 1건·주문 입력 WAITING·공개 export 변경 없음으로 확인됐다. 이 변경은 자료 발견 경로만 보강하며 B2 표시 승인이나 공개 문구 갱신을 자동 수행하지 않는다.
