...
로컬 API를 함께 실행하는 운영 화면은 `/api/ops/local-audit`에서 이 패킷의 안전한 요약을 60초마다 읽는다. 화면에는 완제품 후보·자료 누락·주문 파일 수·1500 과거 집계·로컬 감사 판정만 표시하고, 원문 행·개인정보·파일 경로는 응답에서 제거한다. 스냅샷이 없거나 갱신에 실패하면 마지막 요약을 보존하고 watcher 실행 안내를 보여 주며, GitHub Pages처럼 API가 없는 정적 환경에서는 이 요청을 만들지 않는다.
