...
배포 전에는 `pnpm run preflight:deploy`로 Wrangler 설정, production 산출물, 공개 export 범위와 Cloudflare 필수 Secrets의 존재 여부를 한 번에 확인한다. 결과는 비밀값을 출력하지 않고 `READY` 또는 `WAITING`과 누락된 이름만 표시하며, `--strict` 모드에서는 하나라도 준비되지 않으면 실패한다.
