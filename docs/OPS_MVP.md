...
Pages 배포 뒤에는 `validate-live-public.mjs`가 공개 URL을 재시도하며 페이지 응답, 1500 단일 제품, 스마트스토어 목적지, 750 제거, 네 연구 축, provenance와 5개 스트림·12개 작업 운영 큐 일치를 직접 확인한다. `operations-queue.json`은 현재 Goal Contract의 담당·검증·대기 입력만 공개 화면에 제공하고 내부 증거·경로는 포함하지 않는다.
