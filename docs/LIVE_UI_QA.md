...
Yoto 연구 카드의 제목을 `머리를 많이 쓴 뒤에도 뇌파와 활력이 더 유지됐어요`로 바꿔 결과 방향을 긍정형으로 먼저 보여준다. 비교 캡슐·연구 조건·제품 범위는 세부 설명과 연구 조건 카드에서 계속 확인할 수 있다. production build·UI 계약·110개 회귀 테스트와 PR 필수 검사를 통과했으며, Pages에 후보가 반영된 뒤 모바일·데스크톱에서 실제 카드 줄바꿈을 다시 확인한다.
