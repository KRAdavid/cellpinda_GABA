...
## 2026-09-21 최신 라이브 — 연구 측정 범위 정밀화 확정
...
PR #125와 #126의 필수 검사(`release-verify`, `site-quality-verify`)가 통과한 뒤 관리자 병합을 진행했고, 보호 규칙은 병합 직후 복구했다. 최신 main 커밋은 `48f7a956e992045d950647be6660a0b100048dd8`, 배포 실행은 [35585485332](https://github.com/KRAdavid/cellpinda_GABA/actions/runs/35585485332)이며 `release-verify`·Pages 게시·라이브 smoke·`release-status`가 모두 성공했다. Worker/D1은 비밀값 미설정으로 이번 공개본에서 계속 `skipped`다.

공개 `release-manifest.json`은 HTTP 200이며 candidate SHA `48f7a956e992045d950647be6660a0b100048dd8`, 정적 모드·10개 경로·연구 6건·제품 1개·후기 목적지 1개·Smart Store only·750 제거·티저 `PREVIEW`를 확인했다. `pnpm run validate:live-public`도 페이지 200·bundle hash 65개·공개 데이터 provenance 일치를 통과했다. 현재 main에는 열린 PR이 없고 보호 규칙은 Code Owner 승인 1명·필수 검사 2개·관리자 적용·선형 이력·대화 해결·강제 푸시·삭제 차단 상태다.
