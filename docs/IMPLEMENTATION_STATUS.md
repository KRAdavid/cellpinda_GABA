...
승인 연구 8건의 공개 문장을 관찰된 변화·연구 조건 중심으로 다시 작성해 `제한적 근거`, `연구 간 결과가 일치하지 않았고`, `정량 메타분석은 수행하지 않았습니다`처럼 소비자의 선택을 위축시키는 문장을 공개 카드에서 제거했다. 연구 카드는 `연구를 쉽게 보기 → 연구 조건과 관찰 수치` 순서로 숫자와 비교 조건을 보여준다. 내부 원장과 감사용 `limitations`는 보존하되 공개 카드의 한계 목록은 노출하지 않는다. `validate-research-copy.mjs`는 `publicText`, `consumerScope`, `consumerSummary`, `hopefulTakeaway`, `productApplicability`, `result`를 소비자 문장으로 검사한다.
