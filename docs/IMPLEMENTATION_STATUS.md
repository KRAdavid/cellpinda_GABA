...
이번 상단 포인터에 포함된 최신 병합 감사 이력은 PR [#184](https://github.com/KRAdavid/cellpinda_GABA/pull/184)·[#185](https://github.com/KRAdavid/cellpinda_GABA/pull/185)·[#186](https://github.com/KRAdavid/cellpinda_GABA/pull/186)·[#187](https://github.com/KRAdavid/cellpinda_GABA/pull/187)·[#189](https://github.com/KRAdavid/cellpinda_GABA/pull/189)이다. #185의 merge SHA는 `edd84ec072228ef4f641af0f5fbdf577f0623ed5`, #186은 `883800cd10aaba214da3bc3bc8be127d2ae2a181`, #187 병합 뒤 직전 포인터의 main·live SHA는 `58bb9c084b4d36d71f62a76621e0d99d12b6d970`와 deploy run `35667945940`이었다. #189 병합 뒤 현재 main·live SHA는 `4ee76f7768bbca95519a961c5973ff7d505f272e`, 라이브 manifest 생성 시각은 `2026-09-21T23:50:54.184Z`, 최신 성공 배포는 [run 35669329445](https://github.com/KRAdavid/cellpinda_GABA/actions/runs/35669329445)이다. PR #189는 첫 연구 노출에서 일반 GABA 연구와 셀핀다 완제품 시험의 차이를 표시하고, Yamatsu 수면 단계의 소비자 언어를 통일하며, Heba 연구의 측정 대상을 `손끝 감각 점수`로 구체화했다. 이전 값과 현재 값 모두 문서 작성 시점의 역사 스냅샷이며 현재 포인터로 재사용하지 않고, 현재 판정은 라이브 manifest와 최신 성공 배포를 다시 확인한다.

다섯 PR 모두 작성자와 병합자가 `KRAdavid`이고 독립 리뷰 0건·`REVIEW_REQUIRED` 상태에서 관리자 병합됐다. 현재 [CODEOWNERS](../.github/CODEOWNERS)는 `@KRAdavid` 단일 계정만 지정하므로 독립 Code Owner 승인이 없었으며, 이 관리자 병합은 독립 승인을 대체하지 않는다.
...
## 운영 패키지·Worker dry-run — 2026-09-20 현재 후보
...
## 완제품 자료 재대조 — 2026-09-20 `f97a149`

단상자·3개입 샘플·낱포 도면·시험성적서·품목제조보고서를 시각 자료와 텍스트 추출로 교차 확인했다. `기타가공품`, `1.5 g×30포`, `1일 1포`, 섭취·보관 주의사항 표기와 참고용 시험 수치를 확인했지만, 최종 인쇄 승인본·현행 판매 SKU·로트별 적용을 확인한 것은 아니다. 따라서 사이트에는 기존의 보수적 제품 표기만 유지하고, 참고용 시료 수치나 인쇄 샘플을 완제품 효능·보장 함량으로 확장하지 않았다. 상세 대조 내역은 [B2 검증 패킷](B2_MATERIAL_VERIFICATION_20260911.md)에 기록했다.

## TF heartbeat 재검토 — 2026-09-20 `28fa4fe`

TF pulse heartbeat를 갱신했다. 이전 상태 지문과 동일해 `stateChanged=false`를 유지했고, 역할군 8/8·활성 작업 5건·입력 게이트 4건을 다시 검증했다. 자동 실행은 내부 검증과 회의 안건 갱신까지만 수행하며, 외부 표시 승인·후기 권한·티저 공개·Cloudflare 운영 설정·실구매 대사는 사람 게이트에서 멈춘다.

## 공개 동기화 재확인 — 2026-09-20

공개 Pages 루트는 HTTP 200으로 응답했지만 `/release-manifest.json`은 HTTP 404였다. 루트 HTML에는 스마트스토어 1500 링크와 750·공식몰 문자열 미노출은 확인됐지만, 최신 후보의 `뇌컨디션 확인 챌린지`와 연구 경계 문구는 아직 포함되지 않았다. 따라서 공개본은 이전 배포본으로 판단하며, PR 병합 뒤 후보 SHA·route·manifest·핵심 문구를 다시 대조하기 전에는 공개 배포 완료로 기록하지 않는다.

## 최종 후보 보정 — 2026-09-20

소비자 화면의 연구 조건을 `연구에서 먹은 양`과 `셀핀다 제품에 적힌 양`으로 나눠 표시해 연구량이 제품 섭취량처럼 읽히지 않게 했다. 운동 연구는 운동 없이 쉰 조건과 운동 조건을 함께 보여 주고, 손끝 감각 연구는 GABA를 먹지 않고 뇌 속 신호와 손끝 연습을 살펴본 자료임을 분명히 했다. 홈 하이라이트·연구 목록·자바스크립트 미사용 fallback의 문구와 검증 규칙을 같은 원장 기준으로 맞췄다.

배포 경로는 실제 Vite 해시 파일명의 `_`를 포함한 장기 캐시, `/api/content`의 5분 재검증 캐시, 명시적으로 설정한 `MEMBER_ORIGIN`의 CORS만 허용하도록 보강했다. 제품 공유 메타데이터의 사이트명은 정적 페이지와 같은 `셀핀다 발효가바`로 통일했고, 모바일 제품 카드에서 `가격·재고` 라벨이 끊기지 않도록 보정했다. 별도 frontend origin의 cross-site 요청은 `MEMBER_ORIGIN`과 Origin이 일치할 때만 허용한다.

최종 후보는 107개 테스트, 타입검사, 연구 카피·UI·공개 export 검증, production build, 10개 정적 route와 65개 release manifest 파일 검사를 통과했다. GitHub Pages 공개본은 후보와 자동으로 동기화되지 않으므로, PR 승인·main 병합 뒤 후보 SHA와 공개 manifest를 다시 대조해야 배포 완료로 판정한다.

## 최신 연구 카드 문구 통합 — 2026-09-20 `431eced`

홈 연구 하이라이트와 자바스크립트 미사용 연구 화면에서 카드마다 반복되던 제품 경계 문구를 제거했다. 각 카드는 참여자·연구에서 먹은 양·연구 조건만 보여 주고, 섹션 상단에서 한 번만 `연구에서 먹은 양은 셀핀다 제품에 적힌 양과 달라요`라고 안내한다. 연구 원장 시각화의 참여자 표기도 같은 기준으로 정리했으며, release manifest와 라이브 smoke 검증기가 새 경계 문구를 확인하도록 맞췄다.

## 라이브 동기화 재검증 — 2026-09-20

후보 `431eced`를 공개 Pages에 다시 대조했다. 홈·연구·제품·뇌컨디션 경로는 HTTP 200으로 응답했지만 후보 SHA를 포함하지 않았고, `/release-manifest.json`은 HTTP 404였다. `pnpm run validate:live-public`도 12회 확인 뒤 동일한 manifest 404로 종료했다. 따라서 현재 공개 주소는 후보와 동기화되지 않은 이전 배포본으로 판정한다.

정적 Pages는 현재 공개 가능한 상태지만, 실제 게시를 완료하려면 PR의 Code Owner 승인을 거쳐 `main`에 병합해야 한다. Worker 배포는 별도로 Cloudflare 토큰·계정·D1·관리자 토큰·회원 origin 설정이 필요하다. 이 두 경로를 하나의 완료 상태로 합쳐 판정하지 않는다.

## 공개 Pages 승격 smoke 재검증 — 2026-09-20

공개 주소에 `pnpm run validate:live-public`를 실행했다. 12회 재시도 후 `/release-manifest.json`이 HTTP 404를 반환해 실패했으며, 공개 루트는 후보 manifest를 제공하지 않는 이전 배포본으로 판정했다. 후보 품질 검증과 공개 승격 검증을 분리해 기록하며, PR 승인·`main` 병합 뒤 같은 검사를 다시 실행한다.
...
홈 연구 하이라이트의 결과 문장을 하드코딩하지 않고 승인된 `data/content-ledger.json`의 `consumerSummary`·`consumerHighlight`를 읽도록 연결했다. 수면·뇌파·성장호르몬·손끝 감각 연구는 관찰된 기록과 연구 조건을 분리해 표시하며, 운동 연구는 운동 없이 쉰 조건과 운동 조건을 구분한다. `consumerHighlight`의 누락·제품 효능 보장·과도한 개선 표현은 연구 카피와 공개 export 검증에서 차단한다.

연구 경로의 주제 필터에 `손끝 감각`을 추가하고, 자바스크립트가 꺼진 홈·연구 fallback에도 승인된 6개 연구 축(수면 2건, 스트레스·잠 종합 자료, 뇌파 과제, 운동, 손끝 감각)을 요약한다. Worker 제품 공유 메타데이터는 `/products/` canonical, 제품 구성 이미지, `og:type=product`, 이미지 MIME·대체텍스트를 사용한다. Worker 루트 배포 시 404 경로의 GitHub Pages prefix가 남지 않도록 origin 재작성도 보완했다.
...
## 최신 브라우저 런타임 감리 — 2026-09-20

production preview를 Chrome CDP로 390px 모바일 viewport에서 다시 열어 홈·연구·뇌컨디션 챌린지 경로를 확인했다. 홈의 1분 체크와 챌린지 CTA, 연구 경로의 연구 카드 6건, 챌린지의 `먼저 연습하고 시작하기`가 모두 표시됐고 세 경로의 가로 넘침은 0건, 콘솔 오류는 0건, 비동기 로딩 상태 잔류는 0건이었다. 이는 로컬 후보의 실제 렌더링 증거이며, 아직 후보와 동기화되지 않은 GitHub Pages 공개본의 품질을 의미하지 않는다.
...
연구 화면뿐 아니라 제품·후기·챌린지·스토리·티저·공유 등 첫 화면 아래의 비핵심 섹션도 지연 로드하도록 분리했다. 연구 전용 CSS 역시 연구 경로에서만 불러온다. production build 기준 초기 JavaScript는 370KB에서 299.47KB( gzip 91.84KB ), 초기 CSS는 약 143KB에서 85.97KB( gzip 16.49KB )로 줄었다. 각 지연 로드 영역에는 시각적 로딩 스켈레톤과 보조기술용 상태 문구를 두어 첫 화면이 빈 상태로 남지 않게 했다. 연구 경로 진입 후 카드 6건과 소비자 문구가 정상 표시되고, 390px 모바일 화면 가로 넘침이 0건임을 Chrome CDP로 확인했다. 타입검사·107개 테스트·UI 계약·production build·필수 PR 검사를 통과했다.
...
Browser 플러그인과 Playwright가 연결되지 않아 로컬 Chrome CDP fallback으로 production preview를 확인했다. 390px 모바일과 764px 데스크톱 viewport에서 제목·첫 CTA·뇌컨디션 확인 챌린지·연구 흐름이 표시되고 가로 넘침은 0건이었다. 모바일에서 `먼저 연습하고 시작하기`를 누르면 `초록 신호를 눌러 보세요.` 화면으로 전환되고 키보드 포커스가 연습 안내에 이동했다. 사용자 제스처 전 Web Audio 차단 경고 1건은 브라우저 정책에 따른 정상 경고이며, 화면에는 효과음 차단 안내를 제공한다. 이 검증은 실제 기기·브라우저 조합 전체를 대신하지 않으며, Pages 배포 후 동일 흐름을 live smoke에서 다시 확인한다.

추가로 `/focus/`, `/research/`, `/products/` 진입 경로를 390px·1440px에서 확인했다. 세 경로 모두 가로 넘침이 없었고, 연구·제품 경로의 canonical과 핵심 CTA가 표시됐다. 공유용 focus 페이지와 본문 챌린지의 시작 문구를 `먼저 연습하고 시작하기`로 통일해 정적·동적 진입 경험의 차이를 줄였다.

## 소비자 문구·접근성 보정 — 2026-09-20

연구 경로의 첫 제목을 `사람 연구 결과를 한눈에 보기`로, 제품 안내를 `가바 1500 한 상자 구성을 확인하세요.`로 바꿔 방문자가 결과와 다음 행동을 바로 이해하게 했다. 연구 카드 상세 열기는 `이 연구, 어떻게 했나요?`, 뇌 피로 연구 묶음은 `관련 연구 5편 더 보기`로 줄였고, 제품 경계는 `잠·스트레스·집중을 이해하는 연구예요. 제품 정보는 따로 보여드려요.`로 정리했다.

공개 배포 감리에서 확인한 작은 글자의 색 대비를 높이고 CTA 장식 아이콘을 보조기술에서 숨겼다. 홈·제품·연구·챌린지·공유 정적 경로에는 Open Graph 이미지 유형·대체텍스트와 Twitter 이미지 대체텍스트를 모두 생성하도록 바꿨으며, 정적 번들 검증기가 누락을 차단한다. 390px Chrome CDP에서 연구·챌린지 경로의 새 문구와 가로 넘침 0건을 확인했다.

UI 계약도 이 두 색상 토큰과 CTA 아이콘의 `aria-hidden`을 직접 검사해 이후 TF 보정에서 다시 누락되지 않게 했다.

## 게임 시작 선택지 문구 보정 — 2026-09-20
...
챌린지의 `fatigue_game_practice_start` 이벤트가 프런트에서 발생하지만 서버·Worker 허용 목록에서 누락돼 있던 모니터링 공백도 보완했다. 관리자 화면 라벨과 Node/D1 회귀검사를 함께 추가해 연습 시작 → 본 게임 시작 → 휴식 후 재점검 흐름을 같은 익명 이벤트 체계에서 확인할 수 있게 했다.
...
## 이전 후보 보정 이력 — 2026-09-20 후보 `1be912c`

연구 카드의 반복 마케팅 문구를 주제별 소비자 행동 문장으로 교체했다. 대표 연구 카드에는 `오늘 연결해 보기`를 기본 화면에 표시하고, 나머지 연구의 다음 행동은 상세 영역 안에서만 보여 카드마다 같은 문장이 반복되지 않는다. 승인 연구의 다음 행동 문구가 중복되면 `validate:research-copy`가 실패하도록 계약을 추가했다. 연구 경계 설명은 연구 목록 안내 박스 한 곳에 두고 소개 문단에서는 같은 문구를 반복하지 않는다.

해당 후보 검증은 `pnpm test` 107개, 타입검사, 연구 카피·UI 계약, production build와 공개 산출물 검사를 모두 통과했다. 이후 후보 `653d25b`에서 소비자 연구 문구와 회귀 검증을 추가로 보정했다.

## 최신 UX 보정 — 2026-09-20

`뇌컨디션 확인 챌린지` 첫 결과 화면을 기록 구간별 안내로 보정했다. 높은 기록에는 안정적인 반응 흐름을 축하하고, 중간 기록에는 5분 뒤 재점검을 제안하며, 낮은 기록에는 화면·알림을 내려놓고 5분 쉬도록 안내한다. 모든 상태에서 점수는 해당 게임의 맞힌 비율일 뿐 건강 상태 측정값이 아니라는 문구를 함께 보여 준다. 결과 링·안내 패널·기록 마치기 화면이 같은 상태를 공유해 시각적 의미가 어긋나지 않는다.
...
제품 직접 진입 메타데이터 보완 커밋 `78434f8`을 Pages에 배포했다. `?view=products#products`와 `/products/` 경로는 브라우저 제목 `셀핀다 가바 1500 · 30포 구성 보기`, 제품 설명, 구성 대표 이미지, `/products/` canonical을 사용한다. Chromium CDP에서 라이브 화면의 제품 카드·30포·기타가공품·스마트스토어 가격·재고 CTA와 함께 이 메타데이터를 확인했다. UI 계약은 같은 기준이 다시 홈 메타데이터로 돌아가지 않도록 회귀 검사를 포함한다.

공개 콘텐츠 기준 main `7f95e9f`를 포함한 배포 검증 run `35438769287`이 통과했다. 이후 감사 문서만 갱신한 main 커밋은 `ea514fd`다. Pages 게시·라이브 smoke·release status 기록까지 완료했으며, 라이브 smoke는 페이지 200, 연구 6건, 공개 claim 12개, 제품 1개, 공유 페이지 6개, 스마트스토어 목적지, 750 제거, provenance 일치를 확인했다. 소비자 화면의 제품 이미지에는 제품 구성과 식품 유형을 설명하는 대체텍스트를 강제하는 회귀 검사를 추가했다. Yoto 연구 요약과 시각화는 연구 조건을 유지한 소비자 문장으로 동기화됐고, 연구 카드의 상세·출처·연구 배경 라벨을 생활 언어로 정리했다. 티저는 승인 전 `HOLD`이며 공개 미디어 URL·iframe을 노출하지 않는다. Worker/D1, 최종 제품 표시·SKU, 후기 재게시 권한, 주문·환불 대사와 티저 자막·권리·CTA는 사람 입력 게이트로 남아 있다. Chromium headless CDP로 로컬 production preview와 공개 연구 경로를 1440px·390px에서 재감리했고 가로 넘침과 콘솔 오류가 없음을 확인했다. 이 화면 검증은 실제 기기별 사용성 조사나 외부 전문가 인증을 대신하지 않는다.
...
공개 소비자 화면과 정적 제품 페이지의 모든 이미지에 명시적인 대체텍스트가 있는지 `validate:ui-contract`에서 검사한다. 제품 이미지는 `셀핀다 가바 1500, 30포 한 상자 구성, 기타가공품 식품 유형`으로 읽히며, 장식용 이미지는 빈 대체텍스트를 허용해 스크린리더가 불필요한 장식을 반복하지 않게 했다. 이 기준은 최신 main `7f95e9f`와 배포 검증 run `35438368577`에 포함됐다.

### 최신 업데이트 — 2026-09-19 연구 카드 소비자 라벨

연구 카드 상세 열기 문구를 `누가·어떻게·무엇을 봤는지 보기`로, 출처 영역을 `출처와 연구 배경`으로, 지원 관계 안내를 `연구를 지원한 곳·연구자 소속`으로 통일했다. UI 계약과 연구 카피 검증이 이 세 라벨의 재유입을 검사하며, 공개 라이브 경로에서 실제 문구를 다시 확인했다.
...
외부 티저 iframe은 리듬 체크 다음 구역에 접근하면 `autoplay` 권한과 함께 로드되며, 영상 화면 자체가 자동 재생을 시도한다. 사이트 카드가 재생 버튼을 먼저 누르라고 안내하던 문구를 `화면에 들어오면 자동 시작`과 `이 화면에 들어오면 영상이 자동으로 시작돼요`로 맞췄다. 브라우저 정책으로 자동 시작이 막히는 경우에는 화면 안 재생 버튼과 새 창 경로를 보조 수단으로 남겼다. UI 계약·티저 경계·라이브 smoke에서 이 문구와 fallback을 검사하도록 고정했으며, 티저 권리·자막·표시·최종 공개 승인 상태는 기존 WAITING 게이트를 유지한다.

## 최신 업데이트 — 2026-09-19 소비자 첫 화면 번들 분리

로컬에서만 사용하는 `OperationsMvp`를 소비자 첫 화면 번들에서 지연 로드하도록 바꿨다. `/ops` 내부 화면에 접근할 때만 운영 UI JavaScript와 CSS를 불러오고, 일반 공개 화면은 운영 큐·회의 UI를 내려받지 않는다. 운영 경로는 기존처럼 개발 환경과 로컬 호스트에서만 열리며, 로딩 중에는 `운영판을 여는 중입니다.` 상태를 보여 준다. production build에서 운영 청크가 별도 생성되고 공개 export·UI 계약·타입검사를 통과했다.

## 최신 업데이트 — 2026-09-19 소비자 화면·원장 재동기화

결과 화면의 반복 설명을 줄여 답변 요약·한 가지 휴식 제안·친구 초대가 한 화면에서 이어지게 했다. 초대 링크는 친구의 기록이 아니라 챌린지 초대라는 점을 분명히 하고, 연구 요약은 일반 GABA 연구와 셀핀다 제품 정보를 쉬운 문장으로 구분했다. 제품 영역 상단에는 스마트스토어 가격·재고와 구매자 후기 바로가기, 연구 카드에는 연구비·저자 소속 공개 문구를 배치했다. 예전 자동 동기화가 만든 승인 문구만 현재 원장으로 다시 맞추도록 Node·Worker 저장소를 보강하고 회귀 테스트를 추가했다. 운영자가 직접 검토한 문구와 보류 상태는 그대로 보존한다.
...
## 최신 업데이트 — 2026-09-12 TF heartbeat 갱신

`pnpm run tf:pulse:heartbeat`를 실행해 활성 Goal Contract와 업무 그래프를 다시 읽고, 2026-09-11T19:03:18Z 기준 pulse를 기록했다. 상태 지문은 이전과 같아 `stateChanged=false`를 유지했고, 검증 대기 1건·외부 입력 게이트 4건·DONE 8건을 다음 회의 큐에 보존했다. 자동 heartbeat는 공개·구매·법적 약속을 실행하지 않으며, 사람 검토가 필요한 B2·B3·B4·C2·E1을 자동 승인하지 않는다.

## 최신 업데이트 — 2026-09-12 반복 원장 동기화 경계 보강

첫 번째 원장 동기화가 revision을 올린 뒤에도 다음 원장 변경을 다시 반영할 수 있도록 Node SQLite와 Worker/D1의 seed 행 판정을 감사 이력과 연결했다. 마지막 변경이 `Current source ledger reconciliation ...`인 행은 다음 seed에서도 갱신하고, 운영자 수정 이력이 있는 행은 유지한다. 제거된 공개 행은 계속 `hold`로 전환하며, 모든 변경은 audit에 남긴다. Node 10개·Worker 11개 테스트로 반복 동기화와 운영자 수정 보존을 확인했고, 공개 제품·후기·연구 승인 상태는 변경하지 않았다.
...
## 최신 업데이트 — 2026-09-12 공개 호스트 관리자 경로 차단

`?view=admin`·`/admin`을 공개 호스트에서 직접 열어도 내부 콘텐츠 검토실이 렌더링되지 않도록 로컬 호스트 조건을 관리자 경로에도 적용했다. 로컬 운영 주소에서만 역할별 로그인 화면을 사용할 수 있고, GitHub Pages에서는 소비자 화면으로 돌아간다. UI 계약 검사에 관리자 경로의 로컬 경계를 고정했다.

## 최신 업데이트 — 2026-09-11 로컬 자료·주문 입력 재감사

제품 자료 폴더를 읽기 전용으로 다시 스캔해 7개 파일(완제품 후보 5개, 제외 벌크 자료 1개)을 확인했다. 새 표시 승인본이나 제품 분류·SKU 확인은 추가되지 않아 B2는 `VERIFYING`을 유지한다. 주문 자료는 1,086개 파일을 다시 확인했고, 1500 후보 114행·121개 수량과 750 후보 64행·72개 수량이 집계됐지만 상태·취소·환불 열과 판매자 계정 응답이 없어 E1은 `WAITING`을 유지한다. 원본 개인정보와 750 자료는 공개 export에 포함하지 않는다.

## 최신 업데이트 — 2026-09-11 소비자용 발효 과정 표현 정리
...
## 2026-09-11 공개 연구 문장 중립화 후속 감리

공개 연구 카드의 결과 방향을 선택적으로 강조하지 않도록 잠드는 시간·뇌파·성장호르몬·제지방량 등의 `publicText`와 `consumerSummary`를 연구 질문·비교 조건·측정 내용 중심으로 다시 정리했다. 소비자 화면에는 `result`·`limitations`를 계속 내보내지 않으며, 2020년 문헌고찰의 근거 수준과 원문 대상 표현은 내부 원장에 보존하고 공개용 `population`과 분리했다. `evidenceHash`는 공개 메타데이터와 내부 메타데이터·구조화 근거를 함께 계산한다.

연구 카드 안의 반복적인 제품 이동은 제거하고 모든 승인 연구 뒤에 `셀핀다 제품 구성 확인` CTA를 한 번 배치했다. 상세 영역 제목은 `이 연구를 읽는 방법`으로 정리했다. 후기 자료가 없을 때 메뉴·제품 카드가 빈 `#reviews` 위치로 이동하지 않도록 조건부 연결을 적용했고, 단일 승인 제품에서도 제품 구성 링크 공유가 노출된다. 터치로 이미 선택한 답을 다시 눌러도 다음 질문 자동 진행이 예약되도록 보정했다.

최신 공개 export·라이브 검증에서 부정적 효과 문구, 내부 `result`·`limitations`, 750 제품이 노출되지 않음을 확인했다. 이 공개 문장 편집은 소비자용 요약 범위를 조정한 것이며, 실제 외부 전문가 승인·후기 재게시 권한·Cloudflare Worker 운영 시크릿·스마트스토어 주문 대사를 대신하지 않는다.

## 2026-09-11 7일 리듬 챌린지 공유 루프 보강

7일 기록의 3일·7일 도달 시 생활 실천 요약 카드를 브라우저에서 생성하고, 카드 미리보기·기기 공유·PNG 저장·초대 링크 복사 대체 경로를 제공한다. 카드에는 완료한 날 수와 연속 흐름만 표시하며 개인 메모·문항 답변·제품 효과 판단은 포함하지 않는다. 챌린지 화면에서 `다시 1분 리듬 체크하기`로 돌아가는 링크도 항상 제공한다.

후기 데이터가 없는 응답에서는 GABA FAQ의 사용 경험 링크를 숨겨 빈 앵커 이동을 방지한다. 변경은 `SevenDayChallenge.tsx`, `SevenDayChallenge.css`, `GabaStory.tsx`, `App.tsx`에 반영했고 타입검사·73개 회귀 테스트·production build·GitHub Pages 라이브 smoke를 다시 통과했다.

## 2026-09-11 결과별 SNS 미리보기 경로 보강

결과 공유 URL을 `share/{유형}/` 정적 경로로 만들고 6개 리듬 유형마다 Open Graph·Twitter 제목·설명·공유 이미지를 미리 생성한다. 정적 페이지는 `ref`·`campaign` 같은 허용된 익명 파라미터만 보존해 앱의 1분 체크 화면으로 넘기며, 링크를 연 사람의 답변이나 개인정보를 전달하지 않는다. 앱 내부에서도 정적 경로와 기존 `?rhythm=` 형식을 같은 규칙으로 해석하고, 결과별 OG 메타데이터를 보완한다.
...
카카오톡 공유 카드가 정적 결과 페이지와 동일한 `social-rhythm-{type}.png`를 사용하도록 수정했다. 유형별 공유 URL·OG 이미지·카카오 미리보기가 같은 생활 리듬 이야기를 가리키며, UI 계약 검증이 해당 이미지 경로를 고정한다. 실제 Kakao 키·도메인 등록과 전송 성과는 외부 확인 항목으로 유지한다.

## 2026-09-11 라이브 공개 JSON 부정형 문구 회귀 검사

라이브 smoke가 공유 HTML과 함께 `content.json`·`gaba-master-index.json`을 검사하도록 보강했다. 공개 배포 데이터에 소비자용 부정형 연구 문구가 재유입되면 검증 단계에서 즉시 실패하며, 내부 감사 원장의 상세 결과·한계는 공개 export와 분리한다.

### 라이브 소비자 흐름·콘솔 감리 (2026-09-11)

Chrome 기반 Playwright 임시 실행기로 데스크톱 1440×1000과 모바일 390×844를 점검했다. 첫 화면의 GABA 설명·1분 체크 CTA·제품 미니카드, 가로 오버플로 없음, 다섯 문항 터치 자동 진행(`1→2→3→4→5`), 결과 카드·공유 링크, `share/active/`의 새 체크 버튼을 확인했다. 티저 iframe은 자동재생 권한을 유지하면서 `allowfullscreen` 중복 속성을 제거했고 자동화 콘솔의 앱 오류·경고가 없었다.
## 2026-09-11 후기 읽기 문장 소비자 흐름 보정

후기 카드의 빈 사용 기간 안내를 `원문에서 확인해 보세요`로 바꾸고, `빠진 정보는 추측하지 않아도 됩니다`처럼 방어적으로 들리는 문장을 원문 맥락을 천천히 확인하는 안내로 교체했다. 실제 승인 후기 원문이 확보되기 전까지는 인용 카드를 숨기는 권한·표시 게이트를 유지하며, 검증 스크립트가 이전 표현의 재유입을 차단한다.
## 2026-09-11 공개 링크 응답 점검

공개 콘텐츠와 마스터 인덱스의 HTTPS 링크 13개를 실제 요청으로 확인했다. 연구·특허 출처 12개는 200 또는 203으로 응답했으며 스마트스토어는 네이버 요청 제한으로 429를 반환했다. 스마트스토어 URL은 승인된 호스트·경로와 일치하고, 구매 목적지는 외부 판매자 페이지로 연결되므로 rate limit을 제품 링크 오류로 판정하지 않는다.
## 2026-09-11 소비자 메뉴의 제품 비교 목적지 명확화

상단 메뉴의 제품 목적지를 `제품 비교`로 표시하고, 실제 섹션에서는 현재 승인된 가바 1500 구성·표시사항만 비교 기준으로 안내한다. 750 제품이나 미확인 SKU를 메뉴·카드에 추가하지 않으며 UI 계약 검증에 제품 비교 목적지 검사를 연결했다.
## 2026-09-11 연구 카드의 제품 탐색 다음 행동 보정

승인된 8개 연구의 소비자용 다음 행동 문장을 생활 루틴 → 제품 구성·표시사항 확인 → 개인 선택의 흐름으로 보강했다. 관심을 유도하지만 연구 결과나 연구 용량을 셀핀다 완제품의 효능·권장량으로 말하지 않으며, `validate-research-copy`와 공개 export 검증으로 같은 문구 원칙을 유지한다.

## 2026-09-11 부정형 연구 문구 재유입 감리

소비자 화면과 공개 JSON에서 `스트레스에 제한적`, `수면에 매우 제한적`, `연구 간 결과가 일치하지 않았고`, `정량 메타분석은 수행하지 않았습니다`, `다만 GABA만의 효과인지는 알 수 없습니다`처럼 선택을 위축시키는 문구가 다시 들어오지 않는지 전체 경로를 점검했다. 공개 카드는 연구 질문·참여자·기간·비교·측정 내용을 쉬운 말로 먼저 보여주고, 제품 적용은 제품 구성과 표시사항을 확인하는 다음 행동으로 연결한다. 원문 결과·한계는 내부 근거 원장과 관리자 검토 화면에만 남긴다.

`validate-research-copy`, `validate-public-export`, `validate-live-public`, production build, 73개 테스트가 모두 통과했다. 2026-09-11 14:40 UTC TF pulse도 상태 변화 없이 안전한 내부 실행(`safeExecution: MET`)을 기록했으며, B2·B3·B4·C2·E1 외부 입력 게이트는 사람 검토 대기 상태를 유지한다.

## 2026-09-11 가바 1500 상품 상세 링크 고정

제품·구매·후기 CTA를 스마트스토어 메인 화면이 아닌 사용자가 지정한 가바 1500 상품 상세 페이지 `https://smartstore.naver.com/cellpinda/products/4701017202`로 통일했다. 공개 원장·정적 fallback·콘텐츠 연결 실패 화면·제품 카드·구매 전 질문·후기 목적지 검증과 Worker의 후기 링크 허용 경계를 함께 갱신해, 다른 상품이나 판매처 홈으로 우회하지 않도록 했다. URL 연결은 상품 상세 진입을 보장하지만 재고·가격·주문 대사는 별도 운영 확인 항목이다.
## 2026-09-12 제품·챌린지 공유 캠페인 맥락 보존

제품 구성 공유와 7일 리듬 챌린지 초대 링크가 현재 방문 URL의 승인된 `campaign` 식별자만 이어받도록 보강했다. 식별자는 영문·숫자·`_`·`-` 조합 1~64자로 제한하며 답변·개인정보·보상 정보는 포함하지 않는다. 결과 공유의 `ref`·`campaign` 경계와 제품·챌린지의 캠페인 경계를 각각 UI 계약 검사에 연결해 공유 경로가 늘어나도 익명 측정 범위를 벗어나지 않게 했다.

내부 안전 TF 실행에서 공개 패킷 재생성, 연구 카피, 티저 경계, 샌드박스 MVP, 공개 export, TF pulse를 모두 `MET`으로 확인했다. 실행 후보는 없고 사람 입력 게이트 B2·B3·B4·C2·E1은 유지된다. `pnpm run build`, 76개 회귀 테스트, GitHub Actions와 라이브 smoke가 성공한 커밋에서 이 변경을 배포했다. 캠페인 토큰 경계는 `src/domain/share.ts`의 공통 함수와 전용 회귀 테스트로 관리한다.

## 2026-09-12 공유 미리보기 메타데이터 회귀 가드

라이브 공개 검증기가 루트 페이지와 여섯 결과 공유 경로의 canonical URL, Open Graph 제목·설명·이미지, Twitter 이미지, 앱 handoff를 각각 확인하도록 보강했다. 유형별 `social-rhythm-{id}.png`와 경로가 어긋나거나 설명 메타데이터가 빠지면 배포 검증이 실패한다. 최신 라이브에서 `validate:live`가 HTTP 200·제품 1종·연구 8건·공유 6개·Smart Store 전용·750 제거·provenance 일치를 다시 통과했다.

## 2026-09-12 연구 카드 한눈표 보강
...
## 2026-09-12 내부 TF 경로 공개 호스트 차단

소비자 메뉴에는 노출하지 않던 `?view=ops`가 공개 호스트에서 직접 입력될 때 내부 TF 큐·게이트·회의 패킷을 렌더링할 수 있던 경로를 점검했다. 운영 화면은 `localhost`, `127.0.0.1`, `[::1]`에서만 열리도록 제한해 로컬 운영자의 중간 확인 흐름은 유지하고 GitHub Pages·외부 배포 호스트에서는 소비자 화면만 표시한다. UI 계약 검증에 로컬 호스트 경계와 공개 호스트 비렌더링 조건을 추가했다.

`validate:ui-contract`, `typecheck`, production build를 통과했다. 이 경계는 공개 JSON 자체의 안전한 요약을 제거하는 변경이 아니라, 내부 운영 UI와 소비자 화면을 분리하는 라우팅 보호다.

## 2026-09-12 익명 사용성 측정 선택 구조 추가

소비자가 사용성 측정 여부를 스스로 선택할 수 있도록 하단에 `선택형 익명 사용성 측정` 패널을 추가했다. 허용 전에는 방문·체크·공유·구매 이동 이벤트를 전송하지 않으며, 허용한 경우에도 화면 흐름만 익명으로 기록하고 이름·연락처·문항별 답변은 보내지 않는다. `측정하지 않기`를 선택해도 리듬 체크·연구 읽기·제품 정보·스마트스토어 이동은 그대로 이용할 수 있고, 설정에서 선택을 바꿀 수 있다.

`src/domain/analytics-consent.ts`의 브라우저 저장값과 이벤트 게이트, `AnalyticsConsent` UI, 회귀 테스트를 연결했다. 계약 검사·타입 검사·79개 테스트·production build를 통과했다.

동의 패널은 푸터의 일반 링크와 분리된 전체 폭 행으로 배치해 모바일·데스크톱에서 CTA와 겹치지 않도록 고정했다. UI 계약 검사에 이 레이아웃 조건을 추가했다.

## 2026-09-12 로컬 자료 요청 경로 자동 탐색

`data/local-material-manifest.json`에 사용자가 지정한 자료 경로와 현재 PC에서 발견된 경로를 함께 기록했다. 감사 스크립트는 두 루트를 읽기 전용으로 탐색하고 동일 파일은 한 번만 집계한다. 최신 실행은 완제품 후보 5건·자료 7건을 확인했으며, 원료 벌크 라벨은 계속 제외하고 B2 독립 검증 전 공개 제품 문구는 갱신하지 않는다.

## 최신 업데이트 — 2026-09-19 공개 직전 재감리 반영

소비자·근거·배포 역할의 독립 재감리 결과를 [TF 공개 직전 재감리 기록](TF_RELEASE_AUDIT_20260919.md)에 남겼다. 뇌 컨디션 챌린지 첫 화면에 건강 측정이 아니라는 설명을 계속 보이게 했고, 점수 옆 문구를 실제 게임 정답률로 한정했다. 후기 섹션은 구매자 원문이 스마트스토어에서 열린다는 점을 제목부터 알린다. GABA 연구 안내에서 제품 구성으로 바로 가는 링크를 추가했다.

연구 문구는 원문과 다시 대조해 Byun 참여자를 수면 불편을 호소한 성인으로 수정하고 수면의 질(PSQI)·불면 증상(ISI) 선별 기준을 상세 문구에 적었다. Yamatsu 연구는 잠의 질이 낮게 나온 설문 집단임을 요약에 밝혔고, 시각자료가 있는 Byun·Yoto·Yamatsu 카드에서 용량·기간 반복을 줄였다. Powers 연구는 쉬는 조건의 최고 혈액 수치와 운동 후 30분 측정 결과를 쉬운 말로 먼저 보여 주고, 통계 지표 용어는 덜었다. 실제 근육 크기·근력은 측정하지 않았다는 연구 범위는 상세 정보에 남겼다. 연구 결과 전용 링크를 페이지 하단에 배치하고 제품 구성 링크는 연구 카드와 분리했다. 공개 원장 동기화 후 `validate:research-copy`, `validate:ui-contract`, 101개 테스트, production build, 타입검사가 통과했다.

배포 검토는 정적 GitHub Pages가 공개 중인 상태와 Cloudflare Worker/API가 연결되지 않은 상태를 분리한다. 공개 Pages의 `/api/health`·`/api/content`는 404이며, 티저는 `PREVIEW/HOLD`, 실제 후기 인용은 0건이다. 오래된 Git 이력에서 일반·JSON 이스케이프 로컬 경로를 찾는 비차단 감사 스크립트와 3개 회귀 검사를 CI에 추가했다. 현재 스캔에서 11개 상대 파일 경로를 경고했으며, 실제 일치 값은 출력하지 않는다. 이 검사는 이미 공개된 이력을 제거하지 않는다. 이력 재작성·force push는 별도 협의와 승인 전까지 보류한다.

최신 공개 커밋과 라이브 smoke 결과는 배포가 끝난 뒤 추가 확인한다. 정적 Pages 경로의 로컬 렌더링은 검증했지만 실제 기기와 소비자 사용성 검증은 별도다. 전체 목표는 Worker/D1 영구 운영, 실제 후기와 권한, 현행 포장 표시·가격, 티저 영상의 공개 파일·자막·권리 승인, 실구매 대사, 외부 전문가 검토가 남아 미완료다.
...
뇌 컨디션 결과에서 반복되는 요약을 줄이고, 다음 행동과 친구 초대 CTA를 한곳에 모았다. 7일 챌린지 초대는 개인 기록 공유로 읽히지 않게 수정하고, 초대 유입과 5분 휴식 단계에서 키보드·보조기기 초점을 제목으로 이동시킨다. 연구 소개는 짧은 제목과 그림 안내로 줄였으며 모바일 첫 화면에 비교 연구의 두 그룹 수치가 함께 들어오도록 배치했다. 모바일 연구 메뉴가 전역 메뉴 CSS에 가려지던 문제도 별도 검증 대상으로 추가했다. 자동 동기화로 표시된 로컬 DB의 오래된 공개 문구만 승인 원장과 대조해 갱신하고, 변경 이력을 보존했다. 사람 검토가 필요한 Cloudflare Worker/D1 운영 연결과 실제 기기·소비자 검증은 계속 별도 출시 과제로 남긴다.

## 2026-09-20 소비자 결과명·연구 카드 재감리 반영

하루 리듬 결과의 내부형 분류명을 생활 장면 문구로 교체하고, 공유 메타데이터·Worker의 결과 제목·정적 공유 카드 PNG를 같은 매니페스트 기준으로 갱신했다. 결과 점수 그림은 수면과 아침 피로까지 포함한 답변 기록이라는 점이 드러나도록 문구를 조정했다. 연구 목록 제목은 `이 연구에서 본 내용`으로 줄였고, Powers 연구의 설계 요약과 연구에서 관찰된 수치 결과를 서로 다른 영역에 배치했다.

이 변경은 소비자가 링크와 결과 카드를 처음 봤을 때 내부 용어를 해석하지 않아도 되게 하는 편집·시각 개선이다. 연구 수치는 해당 연구의 조건을 설명하는 근거이며 셀핀다 완제품의 효능·권장량·섭취 지침을 새로 승인하지 않는다. `validate:research-copy`, `validate:ui-contract`, 전체 테스트, 타입검사와 production build를 최종 후보에서 다시 실행한다.
...
## 2026-09-20 챌린지 완료 후 다음 행동 연결 보강

뇌컨디션 확인 챌린지의 두 번째 게임 결과에서 공유·재시작 뒤 다음으로 살펴볼 내용을 한 줄 흐름으로 연결했다. 결과 카드는 `GABA 이야기`·`가바 1500 구성`·승인된 스마트스토어 `구매자 후기`를 짧은 3단계 시각 링크로 보여 주며, 제품·후기 목적지는 기존 승인 URL을 그대로 사용한다. 게임 기록과 건강 상태를 혼동하지 않는 안내, 친구 초대, 5분 휴식 비교 흐름은 유지했다. 세 링크는 각각 익명 이벤트만 기록하고 개인 점수·답변은 전달하지 않는다.

UI 계약·타입검사와 정적 route 검증을 통과했으며, 새 후보의 정적 번들에서도 결과 링크가 모바일에서 한 열로 읽히도록 반응형 스타일을 적용했다. 외부 40~50대 사용성 조사와 실제 구매 대사는 여전히 코드로 대체하지 않고 별도 사람 게이트로 유지한다.
