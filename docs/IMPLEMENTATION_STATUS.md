...
승인 연구 8건의 공개 문장을 관찰된 변화·연구 조건 중심으로 다시 작성해 `제한적 근거`, `연구 간 결과가 일치하지 않았고`, `정량 메타분석은 수행하지 않았습니다`처럼 소비자의 선택을 위축시키는 문장을 기본 카드에서 제거했다. 연구 카드는 `연구를 쉽게 보기 → 연구 조건과 근거 자세히 보기` 순서로 숫자와 비교 조건을 보여주며, 상세 영역에는 원문 결과와 한계를 보존한다. `validate-research-copy.mjs`는 `publicText`, `consumerScope`, `consumerSummary`, `hopefulTakeaway`, `productApplicability`를 소비자 문장으로 검사하고 `result`·`limitations`는 근거 상세 필드로 구분한다.
