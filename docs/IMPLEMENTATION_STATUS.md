...
로컬 Node API에 `GET /api/ops/local-audit`를 추가하고 운영 MVP가 60초마다 비공개 감사 요약을 읽도록 연결했다. 카드에는 완제품 후보·자료 확인/누락·주문 파일 수·1500 과거 수량·CSV/XLSX 파싱 상태와 `local-material-inputs`·`local-order-inputs` 판정을 표시한다. 서버는 loopback 요청만 허용하며 원문 행·개인정보·파일 경로·해시를 투영하지 않는다. watcher 스냅샷이 없거나 갱신에 실패해도 기존 요약을 유지하고 실행 안내를 표시하며, GitHub Pages 정적 환경에서는 API 호출을 만들지 않는다.
