...
두 로컬 입력을 전체 목표 판정과 함께 확인하려면 로컬 PC에서 `pnpm run audit:goal:local`을 실행한다. JSON이 필요하면 `pnpm run audit:goal -- --local-inputs --json`을 사용한다. 이 옵션은 `audit-local-materials`와 `audit-local-orders`를 읽기 전용으로 다시 실행해 완제품 후보·제품군·주문 파일 구조·상태 필드 유무·가장 최근 파일 수정 시각을 회의용 JSON에 붙인다. 자료 스캔 성공은 B2 표시 승인, 과거 주문 수량은 E1 실구매·환불 대사를 의미하지 않는다. 결과에는 개인정보·원문 행·로컬 경로를 넣지 않으며, CI 기본 감사는 외부 폴더가 없어도 재현되도록 옵션을 생략한다.
