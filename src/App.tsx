...
function ContentFallback({loading,onRetry}:{loading:boolean;onRetry:()=>void}){
 return <section className="section wrap content-status" aria-live="polite"><p className="chapter">셀핀다 발효가바</p><h2>{loading?'정보를 준비하고 있어요.':'잠시 연결이 늦어졌어요.'}</h2><p>{loading?'제품 구성과 연구 자료를 불러오는 중입니다.':'기본 리듬 체크는 바로 사용할 수 있어요. 아래에서 GABA와 제품 정보를 먼저 살펴보세요.'}</p>{!loading?<div className="actions"><a className="button outline" href="https://smartstore.naver.com/cellpinda/products/4701017202" target="_blank" rel="noreferrer">스마트스토어에서 확인 ↗</a><button type="button" className="button outline" onClick={onRetry}>자료 다시 불러오기</button></div>:null}{!loading?<div className="content-status-grid"><article id="story"><p className="chapter">GABA란?</p><h3>신경 신호의 균형을 돕는 물질</h3><p>GABA는 과도하게 켜진 신경 신호의 강도를 조절하는 데 관여합니다.</p></article><article id="fermentation"><p className="chapter">왜 발효가바?</p><h3>발효에서 품질 확인까지</h3><p>셀핀다는 발효 → 분리·회수 → 함량 확인 → 품질 확인의 흐름으로 제품을 소개합니다.</p></article><article id="products"><p className="chapter">제품 구성</p><h3>셀핀다 가바 1,500 mg × 30포</h3><p>최신 구성과 섭취 정보는 스마트스토어와 포장 표시사항에서 확인해 보세요.</p><a className="text-link" href="https://smartstore.naver.com/cellpinda/products/4701017202" target="_blank" rel="noreferrer">스마트스토어에서 제품 정보 확인 ↗</a></article><article id="reviews"><p className="chapter">구매자 경험</p><h3>스마트스토어 원문에서 확인</h3><p>제품을 선택한 사람들의 경험은 스마트스토어에서 직접 살펴볼 수 있어요.</p></article><article id="research"><p className="chapter">연구를 쉽게 읽기</p><h3>사람 대상 연구를 쉬운 말로</h3><p>GABA와 휴식·수면·운동 관련 연구를 소비자 언어로 정리해 소개합니다.</p><ul><li><strong>스트레스·기분</strong> 과제 상황의 뇌파와 기분 설문을 비교해 본 연구</li><li><strong>수면</strong> 잠드는 시간과 수면 지표를 살펴본 연구</li><li><strong>운동·회복</strong> 성장호르몬과 제지방량 관련 지표를 측정한 연구</li></ul><a className="text-link" href={`${siteRoot}data/gaba-master-index.json`}>GABA 연구를 쉬운 말로 더 보기</a></article></div>:null}</section>;
}
export default function App(){
 ...
 return <><a className="skip" href="#main">본문으로 이동</a><header className="header"><a href={siteRoot} className="brand">Cellpinda<span className="brand-dot">.</span></a><nav aria-label="주 메뉴" className={menu?'open':''} onClick={()=>setMenu(false)}><a href="#rhythm">1분 체크</a><a href="#story">GABA란?</a><a href="#fermentation">왜 발효가바?</a><a href="#products">제품 비교</a>{content?.reviews?.length ? <a href="#reviews">구매자 경험</a> : null}</nav><a href="#rhythm" className="button small" onClick={()=>track('hero_check_start',{path:'/header'})}>1분 체크 <ArrowRight size={18}/></a><button className="menu-toggle" aria-label={menu?'메뉴 닫기':'메뉴 열기'} aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>{linkContext}
 <main id="main"><section className="hero"><img className="hero-photo" src={asset('assets/rhythm-window.png')} alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><p className="chapter">셀핀다 발효가바 · 나의 하루 리듬</p><h1>오늘도 몸보다<br/>머리가 먼저 지치지 않았나요?</h1><p className="hero-question">생각이 멈추지 않는 밤, 쉽게 예민해지는 하루.<br className="mobile-break"/> 1분 리듬 체크로 지금 나에게 필요한 휴식 신호를 확인해 보세요.</p><p className="muted">GABA는 신경 신호의 균형 조절에 관여하는 물질입니다.<br/>셀핀다는 발효기술로 만든 발효가바를 하루 한 포에 담았습니다.</p><div className="actions"><a className="button" href="#rhythm" onClick={()=>track('hero_check_start',{path:'/'})}>1분 리듬 체크 시작 <ArrowRight/></a><a className="button outline" href="#teaser">발효가바 이야기 영상 보기 <ArrowRight/></a></div>{content?.products.length ? <div className="hero-product"><img src={asset(`assets/product-${content.products[0].amountMg}.jpg`)} alt={`${content.products[0].name} 실제 제품 포장`}/><div><strong>김치 유래 유산균 발효기술로 만든 셀핀다 발효가바</strong><span className="hero-product-meta">{content.products[0].name} · {content.products[0].amountMg.toLocaleString()} mg × {content.products[0].servings}포</span><div className="hero-shortcuts"><a href="#products">제품 구성 보기 →</a><a href={content.products[0].officialUrl} target="_blank" rel="noreferrer">구매처 확인 →</a>{content.reviews?.length ? <a href="#reviews">후기 읽기 →</a> : null}</div></div></div> : null}</div></section>
 ...
 <section id="fermentation" className="section sage"><div className="wrap"><div className="section-head"><div><p className="chapter">03 / 왜 발효가바?</p><h2>한 포의 출처를<br/>네 가지 질문으로.</h2></div><p>기술 용어보다 소비자가 확인할 차이를 먼저 살펴봅니다.<br/>자료로 확인되는 내용만 차례로 공개합니다.</p></div><div className="fermentation-questions">{[['무엇으로 만드나요?','셀핀다는 발효 가바 제품으로 소개됩니다.'],['어떻게 만드나요?','공개 특허 문헌에는 유산균에 L-글루탐산을 공급해 GABA를 생산하는 방법이 나옵니다.'],['어떻게 확인하나요?','발효 → 분리·회수 → 함량 확인 → 품질 확인의 흐름으로 자료를 살펴봅니다.'],['왜 셀핀다인가요?','확인된 제품 구성과 표시사항을 스마트스토어에서 비교해 보세요.']].map(([question,answer],index)=><article key={question}><span>0{index+1}</span><h3>{question}</h3><p>{answer}</p></article>)}</div><div className="process">{['발효','분리·회수','함량 확인','품질 확인'].map((t,i)=><div key={t}><span>0{i+1}</span><h3>{t}</h3></div>)}</div>{content.claims.filter(c=>!c.id.startsWith('product-')&&!c.id.startsWith('research-')&&!c.id.startsWith('gaba-')&&c.publicText).map(c=><details className="claim" key={c.id}><summary>{c.publicText}</summary><div>{c.sources.filter(s=>s.url).map(s=><a key={s.url} href={s.url!} target="_blank" rel="noreferrer">공개 자료 확인 · {s.title} ↗ </a>)}</div></details>)}</div></section>
