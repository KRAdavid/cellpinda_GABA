...
const eventMap:Record<string,string>={rhythm_start:'rhythm_check_started',rhythm_complete:'rhythm_check_completed',share_request:'share_requested',share_copy:'share_link_copied',card_download:'share_image_downloaded',purchase_click:'purchase_outbound_clicked',review_open:'review_opened'};
...
export default function App(){
 ...
 return <><a className="skip" href="#main">본문으로 이동</a><header className="header"><a href="/" className="brand">Cellpinda<span className="brand-dot">.</span></a><nav aria-label="주 메뉴" className={menu?'open':''} onClick={()=>setMenu(false)}><a href="#story">GABA 이야기</a><a href="#fermentation">발효기술</a><a href="#products">제품 경험</a><a href="#lab">리듬 연구소</a></nav><a href="#rhythm" className="button small">리듬 체크 <ArrowRight size={18}/></a><button className="menu-toggle" aria-label={menu?'메뉴 닫기':'메뉴 열기'} aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>
 <main id="main"><section className="hero"><img className="hero-photo" src="/assets/rhythm-window.png" alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><h1>오늘, 내 뇌는<br/>쉴 틈이 있었을까?</h1><p className="hero-question">몸은 쉬고 있는데,<br className="mobile-break"/> 머리는 계속 일하고 있나요?</p><p className="muted">바쁜 하루 속, 나의 긴장과 휴식 습관을 돌아보세요.</p><div className="actions"><a className="button" href="#rhythm">1분 리듬 체크 <ArrowRight/></a><a className="button outline" href="#fermentation">발효가바 알아보기 <ArrowRight/></a></div></div></section>
 ...
 <section id="products" className="section wrap"><div className="section-head"><div><p className="chapter">04 / 제품 경험</p><h2>선택에 필요한 차이를,<br/>한눈에.</h2></div><p>1포의 내용량과 구성을 비교하세요.<br/>리듬 체크 결과로 제품이나 용량을 권하지 않습니다.</p></div><div className="products">{content?.products.map(p=><article className="product" key={p.id}><img src={`/assets/product-${p.amountMg}.jpg`} alt={`${p.name} 실제 제품 포장`} loading="lazy"/><div className="product-body"><h3>{p.name}</h3><dl><div><dt>1포 내용량</dt><dd>{p.amountMg.toLocaleString()} mg</dd></div><div><dt>구성</dt><dd>{p.servings}포</dd></div><div><dt>총 내용량</dt><dd>{p.totalG} g</dd></div></dl><a className="button outline" href={p.officialUrl} target="_blank" rel="noreferrer" onClick={()=>track('purchase_click',{productId:p.id})}>공식몰에서 구성·재고 확인 <ArrowUpRight size={18}/></a></div></article>)}</div><p className="note">공식몰 상품명 기준 구성 · 확인일 2026.09.10. 가격·재고·섭취 방법·주의사항은 구매 시 제품 표시사항과 공식몰에서 확인하세요.</p></section>
 <ReviewExperience reviews={content?.reviews??[]} onOpen={productId=>track('review_open',{productId})}/>
 <SevenDayChallenge/>
 <section className="closing"><div className="wrap between"><h2>오늘의 나를 돌아보는 시간.<br/>지금, 시작해 볼까요?</h2><a className="button light" href="#rhythm">1분 리듬 체크 <ArrowRight/></a></div></section></main><footer className="wrap footer"><a className="brand" href="/">Cellpinda.</a><p>하루 리듬을 돌아보고, 충분히 알고 선택하세요.</p><a href="https://cellpinda.co.kr" target="_blank" rel="noreferrer">공식몰 ↗</a><a href="/admin">콘텐츠 검토실</a></footer></>;
}
