...
export default function App(){
 ...
 return <><a className="skip" href="#main">본문으로 이동</a><header className="header"><a href={siteRoot} className="brand">Cellpinda<span className="brand-dot">.</span></a><nav aria-label="주 메뉴" className={menu?'open':''} onClick={()=>setMenu(false)}><a href="#rhythm">잠과 휴식 체크</a><a href="#story">GABA는?</a><a href="#fermentation">발효가바는?</a><a href="#products">제품 구성</a>{content?.reviews?.length ? <a href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>track('review_open',{productId:'gaba1500',path:'/header'})}>가바 1500 구매자 후기 ↗</a> : null}</nav><a href="#rhythm" className="button small" onClick={()=>track('hero_check_start',{path:'/header'})}>1분 체크 <ArrowRight size={18}/></a><button className="menu-toggle" aria-label={menu?'메뉴 닫기':'메뉴 열기'} aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>{linkContext}
 <main id="main"><section className="hero"><img className="hero-photo" src={asset('assets/rhythm-window.png')} alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><p className="chapter">셀핀다 발효가바 · 오늘 내 상태</p><h1>퇴근했는데도<br/>일 생각이 계속 나나요?</h1><p className="hero-question">잠들기 어렵거나, 쉬어도 아침에 피곤한 날이 있나요?<br className="mobile-break"/> 지난 7일을 1분 동안 돌아봐요.</p><p className="muted">GABA는 뇌세포 사이에서 신호를 주고받을 때 쓰이는 물질 가운데 하나예요.<br/>셀핀다는 발효 기술로 만든 GABA를 담았습니다.</p><div className="actions"><a className="button" href="#rhythm" onClick={()=>track('hero_check_start',{path:'/'})}>잠과 휴식 1분 체크 <ArrowRight/></a><a className="button outline" href="#teaser">발효가바 이야기 보기 <ArrowRight/></a></div>{content?.products.length ? <div className="hero-product"><img src={asset(`assets/product-${content.products[0].amountMg}.jpg`)} alt={`${content.products[0].name} 제품 포장`}/><div><strong>발효 기술로 만든 셀핀다 발효가바</strong><span className="hero-product-meta">{content.products[0].name} · {content.products[0].amountMg.toLocaleString()} mg × {content.products[0].servings}포</span><div className="hero-shortcuts"><a href="#products">제품 구성 보기 →</a><a href={content.products[0].officialUrl} target="_blank" rel="noreferrer" onClick={()=>{track('purchase_click',{productId:content.products![0].id,path:'/hero'});track('purchase_cta_click',{productId:content.products![0].id,path:'/hero'})}}>스마트스토어에서 보기 →</a>{content.reviews?.length ? <a href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>track('review_open',{productId:content.products![0].id,path:'/hero'})}>구매자 후기 읽기 →</a> : null}</div></div></div> : null}</div></section>
