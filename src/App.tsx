...
export default function App(){
 ...
 const heroProduct=content?.products.find(product=>product.id==='gaba1500');
 ...
 useEffect(()=>{
  ...
  const description='사람이 GABA를 먹은 연구와 뇌 속 신호를 살펴본 연구를 나눠 소개해요. 셀핀다 가바 1500 제품 정보는 연구와 따로 보여드려요.';
  document.title=title;
  const update=(selector:string,attribute:'name'|'property',value:string)=>{const element=document.head.querySelector<HTMLMetaElement>(`meta[${attribute}="${selector}"]`);if(element)element.content=value;else{const next=document.createElement('meta');next.setAttribute(attribute,selector);next.content=value;document.head.appendChild(next);}};
 ...
 if(researchView)return <><a className="skip" href="#main">본문으로 이동</a><header className="header research-route-header"><a href={siteRoot} className="brand">Cellpinda<span className="brand-dot">.</span></a><nav aria-label="연구 메뉴"><a href={siteRoot}>메인으로 돌아가기</a></nav></header><main id="main" className="research-route-main">{content?<><section className="research-route-intro wrap"><p className="chapter">GABA 사람 연구</p><h1>GABA 사람 연구</h1><p>사람이 GABA를 먹은 연구와 뇌 속 신호를 살펴본 연구를 나눠 소개해요. 셀핀다 가바 1500 제품 정보는 연구와 따로 보여드려요.</p></section><ResearchLibrary claims={content.claims} sectionTitle="주제별로 한눈에 보기" onOpen={()=>track('evidence_opened',{path:'/research'})}/><section className="section wrap research-route-product" aria-labelledby="research-route-product-heading"><div><p className="chapter">제품 구성</p><h2 id="research-route-product-heading">셀핀다 제품 정보</h2><p>셀핀다 가바 1500의 제품 구성과 낱포 표시는 제품 포장에서 확인해 주세요.</p></div><a className="button outline" href={`${siteRoot}#products`}>셀핀다 가바 1500 구성 보기 <ArrowRight size={18}/></a></section></>:<section className="section wrap content-status"><p className="chapter">GABA 사람 연구</p><h1>{loading?'연구 내용을 불러오고 있어요.':'연결이 잠시 늦어졌어요.'}</h1><p>{loading?'사람 연구를 쉽게 정리한 내용을 불러오는 중입니다.':'연구 자료를 불러오지 못했습니다. 다시 시도해 주세요.'}</p>{!loading?<button type="button" className="button outline" onClick={()=>setRetryKey(value=>value+1)}>다시 불러오기</button>:null}</section>}</main><footer className="wrap footer research-route-footer"><a className="brand" href={siteRoot}>Cellpinda.</a><p>GABA 사람 연구 안내</p><a href={siteRoot}>메인으로 돌아가기</a></footer></>;
 ...
 <section id="fermentation" className="section sage"><div className="wrap"><div className="section-head"><div><p className="chapter">03 / 발효가바는?</p><h2>발효가바를<br/>쉽게 알아보세요.</h2></div><p>발효가 무엇인지, 제품 정보를 어디서 볼 수 있는지<br/>쉬운 말로 안내합니다.</p></div><div className="fermentation-questions">{[['무엇으로 만들었나요?','셀핀다 스마트스토어 상품은 발효가바로 소개돼 있어요. 원재료와 함량은 제품 표시사항에서 확인해 주세요.'],['발효 기술이 뭔가요?','특허 문서는 유산균으로 GABA를 만드는 방법을 설명하는 자료예요. 제품의 실제 제조공정은 제품 자료에서 따로 확인해 주세요.'],['제품 정보는 어디서 보나요?','한 포에 든 양과 제품 구성은 제품 포장과 스마트스토어에서 확인할 수 있어요.'],['먹는 법은 어디에 있나요?','제품 포장에 적힌 먹는 방법과 주의사항을 확인해 주세요.']].map(([question,answer],index)=><article key={question}><span>0{index+1}</span><h3>{question}</h3><p>{answer}</p></article>)}</div><p className="process-note"><span>특허 문서의 예시</span>유산균과 재료 성분으로 GABA를 만드는 방법이에요. <a className="text-link" href="https://patents.google.com/patent/KR101740968B1/ko" target="_blank" rel="noopener noreferrer">특허 문서 보기 ↗</a></p><div className="process">{['유산균 + 재료 성분','발효','GABA 생성'].map((t,i)=><div key={t}><span>0{i+1}</span><h3>{t}</h3></div>)}</div>{content.claims.filter(c=>!c.id.startsWith('product-')&&!c.id.startsWith('research-')&&!c.id.startsWith('gaba-')&&c.publicText).map(c=><details className="claim" key={c.id}><summary>{c.publicText}</summary><div>{c.sources.filter(s=>s.url).map(s=><a key={s.url} href={s.url!} target="_blank" rel="noreferrer" aria-label={`${s.title} 문서 보기`}>특허 문서에서 보기 ↗</a>)}</div></details>)}</div></section>
 <section id="products" className="section wrap"><div className="section-head"><div><p className="chapter">04 / 제품 구성</p><h2>가바 1500 한 상자에는<br/>무엇이 들어 있나요?</h2></div><div className="products-intro-actions"><p>상품명과 한 상자 포 수를 확인해 보세요.<br/>낱포 표시·먹는 법·주의사항은 제품 포장에서 확인해 주세요.</p>{content.products.map(product=><a key={product.id} className="button outline products-intro-buy" href={product.officialUrl} target="_blank" rel="noopener noreferrer" onClick={()=>{track('purchase_click',{productId:product.id,path:'/products-intro'});track('purchase_cta_click',{productId:product.id,path:'/products-intro'})}}>스마트스토어에서 가격·재고 보기 <ArrowUpRight size={18}/></a>)}{content.reviews?.length ? <a className="products-review-shortcut" href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>{track('review_open',{productId:'gaba1500',path:'/products-intro'});track('review_source_click',{productId:'gaba1500',path:'/products-intro'})}}>가바 1500 후기 바로 보기 ↗</a> : null}</div></div><div className="products">{content.products.map(p=><article id={`product-${p.id}`} className="product" key={p.id}><div className="product-visual" role="img" aria-label={p.name + ', ' + p.servings + '포 한 상자 구성'}><span className="product-visual-kicker">한 상자 구성</span><strong>{p.servings}<small>포</small></strong><span className="product-visual-subtitle">한 상자에 든 포 수</span><div className="product-portion-grid" aria-hidden="true">{Array.from({length:p.servings},(_,index)=><i key={index}/>)}</div><div className="product-visual-facts"><span><strong>{p.category}</strong> 식품 유형</span></div></div><div className="product-body"><h3>{p.name} <small className="product-category">{p.category}</small></h3><dl><div><dt>가격·재고</dt><dd>{p.availability || '스마트스토어에서 확인'}</dd></div><div><dt>먹는 법·보관</dt><dd>제품 포장에서 확인</dd></div></dl><a className="button outline" href={p.officialUrl} target="_blank" rel="noreferrer" onClick={()=>{track('purchase_click',{productId:p.id,path:'/products'});track('purchase_cta_click',{productId:p.id,path:'/products'})}}>가격·재고 확인하기 <ArrowUpRight size={18}/></a></div></article>)}</div><PurchaseQuestions products={content.products} onEvent={track} />{content.products.length>0&&<ProductShare onEvent={track}/>}</section>
