...
export default function App(){
 ...
 return <><a className="skip" href="#main">본문으로 이동</a><header className="header"><a href={siteRoot} className="brand">Cellpinda<span className="brand-dot">.</span></a><nav aria-label="주 메뉴" className={menu?'open':''} onClick={()=>setMenu(false)}><a href="#rhythm">1분 체크</a><a href="#story">GABA란?</a><a href="#fermentation">왜 발효가바?</a><a href="#products">제품 구성</a>{content?.reviews?.length ? <a href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>track('review_open',{productId:'gaba1500',path:'/header'})}>구매자 후기 ↗</a> : null}</nav><a href="#rhythm" className="button small" onClick={()=>track('hero_check_start',{path:'/header'})}>1분 체크 <ArrowRight size={18}/></a><button className="menu-toggle" aria-label={menu?'메뉴 닫기':'메뉴 열기'} aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>{linkContext}
 <main id="main"><section className="hero"><img className="hero-photo" src={asset('assets/rhythm-window.png')} alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><p className="chapter">셀핀다 발효가바 · 오늘 내 상태</p><h1>일을 마쳤는데도<br/>할 일이 계속 생각나시나요?</h1><p className="hero-question">잠자리에 누워도 잠이 안 오고, 쉬어도 피곤한 날.<br className="mobile-break"/> 1분 체크로 지금 뇌 피로가 쌓였는지 돌아보세요.</p><p className="muted">GABA는 뇌에서 신경세포 사이의 신호를 조절하는 물질입니다.<br/>셀핀다는 발효 기술로 만든 발효가바를 하루 한 포에 담았습니다.</p><div className="actions"><a className="button" href="#rhythm" onClick={()=>track('hero_check_start',{path:'/'})}>1분 리듬 체크 시작 <ArrowRight/></a><a className="button outline" href="#teaser">발효가바 이야기 영상 보기 <ArrowRight/></a></div>{content?.products.length ? <div className="hero-product"><img src={asset(`assets/product-${content.products[0].amountMg}.jpg`)} alt={`${content.products[0].name} 실제 제품 포장`}/><div><strong>발효 기술로 만든 셀핀다 발효가바</strong><span className="hero-product-meta">{content.products[0].name} · {content.products[0].amountMg.toLocaleString()} mg × {content.products[0].servings}포</span><div className="hero-shortcuts"><a href="#products">제품 구성 보기 →</a><a href={content.products[0].officialUrl} target="_blank" rel="noreferrer" onClick={()=>{track('purchase_click',{productId:content.products![0].id,path:'/hero'});track('purchase_cta_click',{productId:content.products![0].id,path:'/hero'})}}>구매처 확인 →</a>{content.reviews?.length ? <a href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>track('review_open',{productId:content.products![0].id,path:'/hero'})}>후기 읽기 →</a> : null}</div></div></div> : null}</div></section>
