...
export default function App(){
 ...
 const linkContext=referralId?<aside className="link-context" aria-live="polite">공유된 리듬 링크로 방문했어요. 내 하루도 1분이면 확인할 수 있어요.</aside>:campaignId?<aside className="link-context" aria-live="polite">캠페인 링크로 방문했어요. 원하는 흐름부터 살펴보세요.</aside>:null;
  ...
  <main id="main"><section className="hero"><img className="hero-photo" src={asset('assets/rhythm-window.webp')} width={1536} height={1024} fetchPriority="high" decoding="async" alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><p className="chapter">나의 하루 리듬 체크</p><h1>퇴근했는데도<br/>일 생각이 계속 나나요?</h1><p className="hero-question">지난 7일, 잠들기 어렵거나 아침에도 피곤한 날이 있었나요?</p><div className="actions"><a className="button" href="#rhythm" onClick={()=>track('hero_check_start',{path:'/'})}>잠과 휴식 1분 체크 <ArrowRight/></a><a className="button outline" href="#teaser">발효가바 이야기 보기 <ArrowRight/></a></div><a className="hero-game-link" href="#focus-game" onClick={()=>track('focus_game_start',{path:'/hero'})}>1분 집중 신호 게임 해보기 <ArrowRight size={16}/></a></div></section>
