...
export default function App(){
 ...
 useEffect(()=>{
  ...
  const image=new URL(asset(focusInvite ? 'assets/focus-game-card-v3.png' : `assets/social-rhythm-${type!.id}.png`),window.location.origin).toString();
 ...
 useEffect(()=>{
  ...
  const description='연구에서 누가 어떤 GABA를 먹고 무엇을 살펴봤는지 쉬운 말과 그림으로 소개합니다.';
  document.title=title;
  const update=(selector:string,attribute:'name'|'property',value:string)=>{const element=document.head.querySelector<HTMLMetaElement>(`meta[${attribute}="${selector}"]`);if(element)element.content=value;else{const next=document.createElement('meta');next.setAttribute(attribute,selector);next.content=value;document.head.appendChild(next);}};
 ...
 const linkContext=referralId?<aside className="link-context" aria-live="polite">공유된 리듬 링크로 방문했어요. 내 하루도 1분이면 확인할 수 있어요.</aside>:campaignId?<aside className="link-context" aria-live="polite">캠페인 링크로 방문했어요. 원하는 흐름부터 살펴보세요.</aside>:null;
  ...
  <main id="main"><section className="hero"><img className="hero-photo" src={asset('assets/rhythm-window.webp')} width={1536} height={1024} fetchPriority="high" decoding="async" alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><p className="chapter">나의 하루 리듬 체크</p><h1>퇴근했는데도<br/>일 생각이<br className="mobile-break"/> 계속 나나요?</h1><p className="hero-question">지난 7일, 잠들기 어렵거나 아침에도 피곤한 날이 있었나요?</p><div className="actions"><a className="button" href="#rhythm" onClick={()=>track('hero_check_start',{path:'/'})}>잠과 휴식 1분 체크 <ArrowRight/></a><a className="button outline" href="#teaser">발효가바 이야기 보기 <ArrowRight/></a></div><a className="hero-game-link" href="#focus-game" onClick={()=>track('focus_game_start',{path:'/hero'})}>1분 신호 반응 게임 해보기 <ArrowRight size={16}/></a></div></section>
 ...
 <section id="products" className="section wrap"><div className="section-head"><div><p className="chapter">04 / 제품 구성</p><h2>가바 1500 한 상자에는<br/>무엇이 들어 있나요?</h2></div><p>한 포에 든 양과 상자 안 포 수를 확인해 보세요.<br/>먹는 법과 주의사항은 제품 포장에 적혀 있어요.</p></div><div className="products">{content.products.map(p=><article id={`product-${p.id}`} className="product" key={p.id}><div className="product-visual" role="img" aria-label={`${p.name}: 한 포 ${p.amountMg.toLocaleString()} mg, ${p.servings}포, 전체 ${p.totalG} g 구성 안내`}><span className="product-visual-kicker">한 상자 구성</span><strong>{p.amountMg.toLocaleString()}<small>mg</small></strong><span className="product-visual-subtitle">한 포 기준</span><div className="product-portion-grid" aria-hidden="true">{Array.from({length:p.servings},(_,index)=><i key={index}/>)}</div><div className="product-visual-facts"><span><strong>{p.servings}포</strong> 한 상자</span><span><strong>{p.totalG} g</strong> 전체 무게</span></div></div><div className="product-body"><h3>{p.name} <small className="product-category">기타가공품</small></h3><dl><div><dt>가격·재고</dt><dd>{p.availability || '스마트스토어에서 확인'}</dd></div><div><dt>먹는 법·보관</dt><dd>제품 포장에서 확인</dd></div></dl>{content.reviews?.length ? <p><a className="text-link" href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>{track('review_open',{productId:p.id,path:'/products'});track('review_source_click',{productId:p.id,path:'/products'})}}>{p.name} 구매자 후기 읽기 →</a></p> : null}<a className="button outline" href={p.officialUrl} target="_blank" rel="noreferrer" onClick={()=>{track('purchase_click',{productId:p.id,path:'/products'});track('purchase_cta_click',{productId:p.id,path:'/products'})}}>가격·재고 확인하기 <ArrowUpRight size={18}/></a></div></article>)}</div><PurchaseQuestions products={content.products} onEvent={track} />{content.products.length>0&&<ProductShare onEvent={track}/>}</section>
