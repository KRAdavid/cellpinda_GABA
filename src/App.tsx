...
export default function App(){
 ...
 <section id="products" className="section wrap"><div className="section-head"><div><p className="chapter">04 / 제품 경험</p><h2>선택에 필요한 차이를,<br/>한눈에.</h2></div><p>현재 판매 제품의 내용량과 구성을 확인하세요.<br/>리듬 체크 결과로 제품이나 용량을 권하지 않습니다.</p></div><div className="products">{content?.products.map(p=><article id={`product-${p.id}`} className="product" key={p.id}><img src={asset(`assets/product-${p.amountMg}.jpg`)} alt={`${p.name} 실제 제품 포장`} decoding="async"/><div className="product-body"><h3>{p.name}</h3><dl><div><dt>1포 내용량</dt><dd>{p.amountMg.toLocaleString()} mg</dd></div><div><dt>구성</dt><dd>{p.servings}포</dd></div><div><dt>총 내용량</dt><dd>{p.totalG} g</dd></div></dl><div className="product-boundary"><p>이 카드에서 확인한 범위</p><ul><li><strong>구성</strong> 확인된 상품명과 제품 이미지 기준</li><li><strong>효능·권장량</strong> 이 카드에서 판단하지 않음</li><li><strong>구매 전</strong> 가격·재고·섭취 방법·주의사항은 스마트스토어와 포장 표시사항 확인</li></ul></div><p><a className="text-link" href="#reviews" onClick={()=>track('review_nav',{productId:p.id,path:'/products'})}>{p.name} 후기 원문 안내 →</a></p><a className="button outline" href={p.officialUrl} target="_blank" rel="noreferrer" onClick={()=>track('purchase_click',{productId:p.id})}>스마트스토어에서 구성·재고 확인 <ArrowUpRight size={18}/></a></div></article>)}</div><p className="note">상품 구성 기준 · 확인일 2026.09.10. 가격·재고·섭취 방법·주의사항은 구매 시 스마트스토어와 제품 표시사항에서 확인하세요.</p><PurchaseQuestions products={content?.products??[]} onEvent={track} />{Boolean(content?.products.length&&content.products.length>1)&&<ProductShare onEvent={track}/>}</section>
 <ReviewExperience reviews={content?.reviews??[]} onOpen={productId=>track('review_open',{productId})}/>
 <SevenDayChallenge/>
 <section className="closing"><div className="wrap between"><h2>오늘의 나를 돌아보는 시간.<br/>지금, 시작해 볼까요?</h2><a className="button light" href="#rhythm">1분 리듬 체크 <ArrowRight/></a></div></section></main><footer className="wrap footer"><a className="brand" href={siteRoot}>Cellpinda.</a><p>하루 리듬을 돌아보고, 충분히 알고 선택하세요.</p><a href="https://smartstore.naver.com/cellpinda" target="_blank" rel="noreferrer">스마트스토어 ↗</a><a href={`${siteRoot}?view=account`}>내 기록</a><a href={`${siteRoot}?view=ops`}>TF 운영판</a><a href={`${siteRoot}?view=admin`}>콘텐츠 검토실</a></footer></>;
}
