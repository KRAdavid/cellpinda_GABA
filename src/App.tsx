...
export default function App(){
 ...
 useEffect(()=>{
  ...
  const title=focusInvite ? '“너도 해봐” 1분 집중 신호 게임 | 셀핀다' : `공유받은 하루 리듬: ‘${type!.name}’ | Cellpinda`;
  const description=focusInvite ? '매번 달라지는 24개 신호에 도전하는 짧은 게임이에요. 내 게임 기록만 확인할 수 있으며 건강 상태를 진단하지 않습니다.' : `친구가 고른 ‘${type!.name}’ 장면을 공유했어요. 내 체크 결과는 아니에요.`;
  ...
  const image=new URL(asset(focusInvite ? 'assets/focus-game-card.png' : `assets/social-rhythm-${type!.id}.png`),window.location.origin).toString();
 ...
 useEffect(()=>{
  ...
  const description='일반 GABA 사람 연구에서 누가 참여했고 무엇을 살펴봤는지 그림과 쉬운 말로 정리했습니다. 셀핀다 완제품을 시험한 결과가 아닙니다.';
  document.title=title;
  const update=(selector:string,attribute:'name'|'property',value:string)=>{const element=document.head.querySelector<HTMLMetaElement>(`meta[${attribute}="${selector}"]`);if(element)element.content=value;else{const next=document.createElement('meta');next.setAttribute(attribute,selector);next.content=value;document.head.appendChild(next);}};
 ...
 return <><a className="skip" href="#main">본문으로 이동</a><header className="header"><a href={siteRoot} className="brand">Cellpinda<span className="brand-dot">.</span></a><nav aria-label="주 메뉴" className={menu?'open':''} onClick={()=>setMenu(false)}><a href="#rhythm">잠과 휴식 체크</a><a href="#story">GABA는?</a><a href={`${siteRoot}research/`}>GABA 연구 읽기</a><a href="#fermentation">발효가바는?</a><a href="#products">제품 구성</a>{content?.reviews?.length ? <a href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>track('review_open',{productId:'gaba1500',path:'/header'})}>가바 1500 구매자 후기 ↗</a> : null}</nav><a href="#rhythm" className="button small" onClick={()=>track('hero_check_start',{path:'/header'})}>1분 체크 <ArrowRight size={18}/></a><button className="menu-toggle" aria-label={menu?'메뉴 닫기':'메뉴 열기'} aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>{linkContext}
 <main id="main"><section className="hero"><img className="hero-photo" src={asset('assets/rhythm-window.webp')} width={1536} height={1024} fetchPriority="high" decoding="async" alt="초록 나무가 보이는 열린 창가와 물 한 잔"/><div className="hero-copy"><p className="chapter">나의 하루 리듬 체크</p><h1>퇴근했는데도<br/>일 생각이 계속 나나요?</h1><p className="hero-question">잠들기 어렵거나, 쉬어도 아침에 피곤한 날이 있나요?<br className="mobile-break"/> 지난 7일을 1분 동안 돌아봐요.</p><p className="muted">잠과 휴식에 관한 최근 일주일을 1분 동안 돌아봐요.</p><div className="actions"><a className="button" href="#rhythm" onClick={()=>track('hero_check_start',{path:'/'})}>잠과 휴식 1분 체크 <ArrowRight/></a><a className="button outline" href="#teaser">발효가바 이야기 보기 <ArrowRight/></a></div><a className="hero-game-link" href="#focus-game" onClick={()=>track('focus_game_start',{path:'/hero'})}>1분 신호 게임 해보기 <ArrowRight size={16}/></a></div></section>
 <section className="intro-strip wrap"><h2>내 하루를 돌아보는 1분,<br/>오늘 쉴 시간을 찾아요.</h2>{[['01','돌아보기','지난 7일, 잠과 휴식은 어땠나요?'],['02','알아보기','GABA와 사람 연구를 쉬운 말로 봐요.'],['03','확인하기','제품 한 포의 양과 먹는 법을 확인해요.']].map(([n,t,d])=><div className="step" key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p></div>)}</section>
 <section className="section empathy" aria-labelledby="empathy-heading"><div className="wrap"><div className="section-head"><div><p className="chapter">오늘 내 하루를 돌아보며</p><h2 id="empathy-heading">이런 날이<br/>있었나요?</h2></div><p>가장 가까운 장면을 골라 보세요.<br/>1분 체크로 이어집니다.</p></div><div className="empathy-cards">{[['퇴근 뒤에도 일이 계속 생각나나요?','일을 끝내고 집에 와도 할 일이 자꾸 떠오릅니다.'],['누워도 잠들기까지 오래 걸리나요?','불을 끄고 누운 뒤 한참 뒤척입니다.'],['하루에 5분도 못 쉬는 날이 있나요?','일과 집안일 사이에 앉아 쉴 틈이 없습니다.'],['아침에 일어나도 피곤한가요?','잠에서 깬 뒤에도 피곤함이 남아 있습니다.']].map(([title,description],index)=><a className="empathy-card" href="#rhythm" key={title} onClick={()=>track('hero_check_start',{path:'/empathy',signal:`0${index+1}`})}><span>0{index+1}</span><h3>{title}</h3><p>{description}</p><strong>지난 7일 돌아보기 <ArrowRight size={16}/></strong></a>)}</div></div></section>
 <div className="wrap section"><RhythmExperience onEvent={track}/></div>
  {content ? <>
   ...
   <section className="section wrap research-gateway" aria-label="GABA 사람 연구">
    ...
    <a className="button outline" href={`${siteRoot}research/`}>연구 페이지 열기 <ArrowRight size={18}/></a>
   </section>
 <section id="fermentation" className="section sage"><div className="wrap"><div className="section-head"><div><p className="chapter">03 / 발효가바는?</p><h2>발효가바를<br/>쉽게 알아보세요.</h2></div><p>발효가 무엇인지, 제품 정보를 어디서 볼 수 있는지<br/>쉬운 말로 안내합니다.</p></div><div className="fermentation-questions">{[['무엇으로 만들었나요?','셀핀다는 발효 기술로 만든 GABA를 담은 제품입니다.'],['발효 기술이 뭔가요?','유산균으로 GABA를 만드는 방법을 특허 문서에서 확인할 수 있어요.'],['제품 정보는 어디서 보나요?','한 포에 든 양과 제품 구성은 제품 포장과 스마트스토어에서 확인할 수 있어요.'],['먹는 법은 어디에 있나요?','제품 포장에 적힌 먹는 방법과 주의사항을 확인해 주세요.']].map(([question,answer],index)=><article key={question}><span>0{index+1}</span><h3>{question}</h3><p>{answer}</p></article>)}</div><div className="process">{['발효하기','걸러내기','한 포에 든 양 확인','품질 살피기'].map((t,i)=><div key={t}><span>0{i+1}</span><h3>{t}</h3></div>)}</div>{content.claims.filter(c=>!c.id.startsWith('product-')&&!c.id.startsWith('research-')&&!c.id.startsWith('gaba-')&&c.publicText).map(c=><details className="claim" key={c.id}><summary>{c.publicText}</summary><div>{c.sources.filter(s=>s.url).map(s=><a key={s.url} href={s.url!} target="_blank" rel="noreferrer" aria-label={`${s.title} 문서 보기`}>특허 문서에서 보기 ↗</a>)}</div></details>)}</div></section>
 <section id="products" className="section wrap"><div className="section-head"><div><p className="chapter">04 / 제품 구성</p><h2>가바 1500 한 상자에는<br/>무엇이 들어 있나요?</h2></div><p>한 포에 든 양과 상자 안 포 수를 확인해 보세요.<br/>먹는 법과 주의사항은 제품 포장에 적혀 있어요.</p></div><div className="products">{content.products.map(p=><article id={`product-${p.id}`} className="product" key={p.id}><img src={asset(`assets/product-${p.amountMg}.jpg`)} alt={`${p.name} 제품 포장`} decoding="async"/><div className="product-body"><h3>{p.name}</h3><dl><div><dt>한 포에 든 양</dt><dd>{p.amountMg.toLocaleString()} mg</dd></div><div><dt>상자 안 포 수</dt><dd>{p.servings}포</dd></div><div><dt>전체 무게</dt><dd>{p.totalG} g</dd></div><div><dt>가격</dt><dd>{p.priceDisplay || '스마트스토어에서 확인'}</dd></div><div><dt>먹는 법·보관</dt><dd>스마트스토어와 제품 포장에서 확인</dd></div></dl><div className="product-boundary"><p>구매 전에 확인해 주세요</p><ul><li><strong>구성</strong> {p.name} {p.amountMg.toLocaleString()} mg × {p.servings}포</li><li><strong>가격·재고</strong> {p.availability || '스마트스토어에서 확인'}</li><li><strong>먹는 법·보관</strong> 스마트스토어와 제품 포장에서 확인</li></ul></div>{content.reviews?.length ? <p><a className="text-link" href={REVIEW_DESTINATION_URL} target="_blank" rel="noopener noreferrer" onClick={()=>{track('review_open',{productId:p.id,path:'/products'});track('review_source_click',{productId:p.id,path:'/products'})}}>{p.name} 구매자 후기 읽기 →</a></p> : null}<a className="button outline" href={p.officialUrl} target="_blank" rel="noreferrer" onClick={()=>{track('purchase_click',{productId:p.id,path:'/products'});track('purchase_cta_click',{productId:p.id,path:'/products'})}}>가격·재고 확인하고 구매하기 <ArrowUpRight size={18}/></a></div></article>)}</div><p className="note">가격과 재고는 스마트스토어에서 확인해 주세요. 먹는 법과 주의사항은 제품 포장에서 확인해 주세요.</p><PurchaseQuestions products={content.products} onEvent={track} />{content.products.length>0&&<ProductShare onEvent={track}/>}</section>
 ...
 <section className="closing"><div className="wrap between"><h2>오늘은 언제 잠깐 쉴 수 있을까요?<br/>1분 체크로 돌아봐요.</h2><a className="button light" href="#rhythm">잠과 휴식 1분 체크 <ArrowRight/></a></div></section></main><footer className="wrap footer"><a className="brand" href={siteRoot}>Cellpinda.</a><p>잠과 휴식에 대해 알아보고, 내게 맞는 선택을 해보세요.</p><a href="https://smartstore.naver.com/cellpinda/products/4701017202" target="_blank" rel="noreferrer">스마트스토어 제품 보기 ↗</a><a href={`${siteRoot}?view=account`}>내 기록</a><AnalyticsConsent/></footer></>;
}
