...
const draft=parseReviewDraft({productId:'gaba750',authorLabel:'가상 테스트 작성자',sourceTitle:'테스트 원문',sourceUrl:'https://example.com/test-review',authoredAt:'2026-09-10',quote:'오직 테스트용 가상 후기',context:'테스트 상황',disclosure:'테스트 제공 관계',rightsEvidence:'PRIVATE RIGHTS',rightsScope:'PRIVATE SCOPE'});
...
test('Korean calendar expiry and future review dates prevent publication',()=>{
 assert.ok(approvalMissing({...draft,rightsExpiresAt:'2026-09-09'},confirmation,['gaba750'],'2026-09-10').includes('rightsExpired'));
 assert.ok(!approvalMissing({...draft,rightsExpiresAt:'2026-09-10'},confirmation,['gaba750'],'2026-09-10').includes('rightsExpired'));
 assert.ok(approvalMissing({...draft,authoredAt:'2026-09-11'},confirmation,['gaba750'],'2026-09-10').includes('authoredDateFuture'));
 assert.ok(approvalMissing(draft,{...confirmation,reviewedAt:'2026-09-11'},['gaba750'],'2026-09-10').includes('reviewedDateFuture'));
});
test('Public quote requires all evidence, confirmations and currently public product; private fields never projected',()=>{
 assert.deepEqual(approvalMissing(draft,confirmation,['gaba750']),[]);
 const item={id:'review-test',status:'approved',reviewType:'quote',review:draft,reviewConfirmation:confirmation};
 assert.ok(publicReview(item,['gaba750']));assert.ok(!JSON.stringify(publicReview(item,['gaba750'])).includes('PRIVATE'));
 assert.equal(publicReview(item,[]),null);assert.equal(publicReview({...item,status:'hold'},['gaba750']),null);assert.equal(publicReview({...item,reviewConfirmation:{...confirmation,rightsConfirmed:false}},['gaba750']),null);
 assert.equal(publicReview({...item,review:{...draft,rightsExpiresAt:'2020-01-01'}},['gaba750']),null);
});
