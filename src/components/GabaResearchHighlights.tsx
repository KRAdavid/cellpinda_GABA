...
const highlightOrder: Highlight[] = [
  {
    ...
    result: 'GABA 조건에서 평균 잠드는 시간이 비교 조건보다 5분 짧게 기록됐어요.',
  ...
  },
  {
    ...
    result: '과제 뒤 뇌파와 활력 점수의 감소 폭이 비교 조건보다 작게 기록됐어요.',
  ...
  },
  {
    ...
    result: '휴식 조건에서 GABA 조건의 최고 수치가 비교 조건보다 높게 기록됐어요. 근육 크기와 근력은 측정하지 않았어요.',
    facts: ['남성 11명', 'GABA 3g 1회', '90분 관찰'],
    Icon: Dumbbell,
  },
];
...
export default function GabaResearchHighlights({claims}: {claims: Claim[]}) {
    ...
    <div className="wrap">
      <div className="section-head gaba-research-highlights-head">
        <div>
          ...
          <h2 id="gaba-research-highlights-heading">티저에서 본 이야기를<br />연구 카드로 바로 확인해 보세요.</h2>
        </div>
        <p>잠·생각·운동 장면에서 일반 GABA를 살펴본 결과를<br />숫자보다 먼저 이해할 수 있게 정리했어요.</p>
      </div>
      ...
      <div className="gaba-research-highlights-grid">
        {highlights.map(({id, label, title, summary, result, facts, Icon}) => <article className="gaba-research-highlight-card" key={id}>
          ...
          <a className="text-link" href={`${import.meta.env.BASE_URL}research/#${id}`}>이 연구 자세히 보기 <span aria-hidden="true">↗</span></a>
        </article>)}
      </div>
      <div className="gaba-research-highlights-footer"><p>연구 조건과 참여자 정보를 더 보고 싶다면 전체 카드에서 확인할 수 있어요.</p><a className="button outline" href={`${import.meta.env.BASE_URL}research/`}>전체 연구 카드 보기 <span aria-hidden="true">→</span></a></div>
    </div>
  </section>;
}
