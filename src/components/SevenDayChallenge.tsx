...
import { CHALLENGE_STORAGE_KEY, challengeHabits, createChallenge, localCalendarDate, parseChallenge, updateChallengeDay, challengeFinished } from '../domain/challenge';
...
export default function SevenDayChallenge({ onEvent }: Props) {
  ...
  return <section id="lab" className="section wrap seven-day-challenge" aria-labelledby="challenge-heading">
    ...
    {!record ? <div className="challenge-start"><div><h3>오늘부터, 일곱 번의 작은 여백.</h3><p>시작일 {today} · 지난 날짜는 나중에 기록할 수 있어요.<br />미래 날짜의 실천은 그날이 오면 기록해 주세요.</p></div><button className="button" onClick={start}>오늘부터 7일 시작 <ArrowRight size={18} aria-hidden="true" /></button></div> : <>
      <div className="challenge-record-heading"><div><p>{record.startDate} ~ {record.days[6]!.date}</p><h3>{completed}일의 실천을 기록했어요.</h3><p className="note">달성 점수가 아닌 완료 표시 수예요. 빠진 날도 편하게 남겨두세요.</p></div><div className="challenge-record-actions"><button className="text-link" onClick={download}><Download size={17} aria-hidden="true" /> 기록 다운로드</button><button className="text-link" onClick={() => setConfirmDelete(true)}><Trash2 size={17} aria-hidden="true" /> 기록 삭제</button></div></div>
      ...
      {finished ? <div className="challenge-wrapup"><p className="chapter">7일을 돌아보며</p><h3>계속하고 싶은 작은 루틴은 무엇인가요?</h3><p>일곱 날짜가 지났어요. 위에 남긴 실천과 메모를 읽고, 내일도 이어가고 싶은 한 가지를 마지막 날 메모에 적어 보세요. 기록하지 못한 지난 날짜도 지금 채울 수 있어요.</p><p className="note">완료 표시가 많거나 적다는 이유로 건강이나 제품의 효과를 판단하지 않습니다.</p></div> : <p className="note">시작일로부터 7일이 지나면 기록을 돌아보는 안내가 열립니다.</p>}
    </>}
