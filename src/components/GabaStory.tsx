...
export default function GabaStory({claims}:{claims:Claim[]}) {
  ...
  return <section id="story" className="section sage"><div className="wrap">
    ...
    <div className="story-grid"><div className="story-steps">
    ...
    </div><div className="signal-demo"><p>신호를 이해하기 위한 개념 모형</p><svg viewBox="0 0 600 160" role="img" aria-label="슬라이더 위치에 따라 달라지는 개념 파동"><path d={Array.from({length:121},(_,i)=>`${i?'L':'M'} ${i*5} ${80+Math.sin(i*.3)*(65-signal*.5)}`).join(' ')} fill="none" stroke="#158457" strokeWidth="3"/></svg><label htmlFor="signal">파동의 움직임을 살펴보세요</label><input id="signal" type="range" value={signal} onChange={e=>setSignal(+e.target.value)}/><div className="between"><span>큰 움직임</span><span>작은 움직임</span></div><small>실제 신경활동 측정이나 제품 섭취 효과를 나타내는 그림이 아닙니다.</small></div></div>
