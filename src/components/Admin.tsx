...
const eventLabels: Record<string, string> = { landing_view: '첫 화면 열람', hero_check_start: '체크 시작 CTA', rhythm_check_started: '리듬 체크 시작', rhythm_check_completed: '리듬 체크 완료', rhythm_check_complete: '리듬 체크 완료', result_viewed: '결과 열람', gaba_story_viewed: 'GABA 이야기 열람', evidence_opened: '근거 상세 열기', research_highlight_opened: '연구 하이라이트 열기', research_library_opened: '전체 연구 카드 열기', review_opened: '후기 원문 이동', review_source_click: '후기 원문 CTA', review_section_navigated: '제품별 후기 섹션 이동', purchase_question_opened: '구매 전 질문 열기', share_image_generated: '공유 이미지 생성', share_requested: '공유 요청', share_cancelled: '공유 취소', share_link_copied: '공유 링크 복사', share_image_downloaded: '공유 이미지 다운로드 요청', result_share_click: '결과 공유 클릭', result_share_success: '결과 공유 처리', shared_link_landed: '공유 링크로 진입', friend_check_start: '친구 체크 시작', product_comparison_viewed: '제품 비교 열람', product_compare_view: '제품 비교 확인', purchase_outbound_clicked: '스마트스토어 구매 링크 이동', purchase_cta_click: '구매 CTA 클릭', challenge_start: '7일 챌린지 시작', challenge_day_complete: '챌린지 하루 완료', seven_day_complete: '7일 챌린지 완료', teaser_impression: '티저 화면 노출', teaser_embed_loaded: '티저 삽입 화면 로드', teaser_external_opened: '티저 새 창 열기', fatigue_game_start: '뇌 피로 반응 게임 시작', fatigue_game_practice_start: '챌린지 연습 시작', fatigue_game_false_start: '반응 게임 너무 이른 입력', fatigue_game_rest_start: '반응 게임 휴식 시작', fatigue_game_complete: '뇌 피로 반응 게임 완료' };
  ...
  <main><h1>콘텐츠 검토실</h1><p>근거와 문구를 확인하고 공개 상태를 관리합니다.</p>
   {!loaded ? <form onSubmit={e => { e.preventDefault(); void load(); }} className="admin-login"><label>운영자 접근 키<input type="password" value={token} autoComplete="off" onChange={e => setToken(e.target.value)} required /></label><p>역할별 접근 키를 입력하면 편집·검토·승인 권한이 자동으로 적용됩니다.</p>{isLocal ? <p className="note">로컬 서버를 사용하는 경우 서버가 생성한 비밀 키 파일에서 접근 키를 확인할 수 있습니다.</p> : null}<button type="submit" className="button" disabled={busy}>검토실 열기</button></form> : <>
    ...
    <div id="content" className="admin-grid">
     <div className="admin-list"><button type="button" className="button outline" disabled={busy || !can('edit')} onClick={() => selectItem(null, true)}>새 인용 후기 등록</button>{items.map(item => <button type="button" key={item.id} className={selected?.id === item.id ? 'selected' : ''} onClick={() => selectItem(item)}><strong>{item.reviewType === 'quote' ? `인용 후기 · ${item.review?.authorLabel || '작성자 미확인'}` : item.topic || item.name || item.id}</strong><span>{item.status === 'approved' ? '공개' : '보류'} · v{item.revision}</span><p>{item.publicText || '공개 문안 미확정'}</p></button>)}</div>
     <section className="admin-edit">{creatingReview || selected?.reviewType === 'quote' ? <ReviewEditor key={`${selected?.id || 'new'}:${reviewSelection}:${selected?.revision || 0}`} item={creatingReview ? null : selected as QuoteReviewItem} api={api} onSaved={reviewSaved} onDirtyChange={setReviewDirty} onRefresh={load} canEdit={can('edit')} canReview={can('review')} canApprove={can('approve')} /> : selected ? <>
      ...
      <div className="actions"><button type="button" className="button outline" disabled={busy || !reason.trim() || !text.trim() || !can('edit')} onClick={() => save()}>수정 저장</button><button type="button" className="button" disabled={busy || !reason.trim() || !text.trim() || !can('approve')} onClick={() => save('approved')}>공개 승인</button><button type="button" className="button outline" disabled={busy || !reason.trim() || !text.trim() || !can('review')} onClick={() => save('hold')}>보류</button></div>
     </> : <p>검토할 문구를 선택하세요.</p>}</section>
    </div>
    <section id="analytics" className="admin-panel"><h2>행동 분석</h2><button type="button" className="button outline" disabled={busy} onClick={() => void load()}>집계 새로고침</button>
