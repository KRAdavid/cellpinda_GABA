...
const eventLabels: Record<string, string> = { landing_view: '첫 화면 열람', rhythm_check_started: '리듬 체크 시작', rhythm_check_completed: '리듬 체크 완료', result_viewed: '결과 열람', gaba_story_viewed: 'GABA 이야기 열람', evidence_opened: '근거 상세 열기', review_opened: '후기 원문 이동', review_section_navigated: '제품별 후기 섹션 이동', purchase_question_opened: '구매 전 질문 열기', share_image_generated: '공유 이미지 생성', share_requested: '공유 요청', share_cancelled: '공유 취소', share_link_copied: '공유 링크 복사', share_image_downloaded: '공유 이미지 다운로드 요청', shared_link_landed: '공유 링크로 진입', product_comparison_viewed: '제품 비교 열람', purchase_outbound_clicked: '스마트스토어 구매 링크 이동' };
   ...
   {!loaded ? <form onSubmit={e => { e.preventDefault(); void load(); }} className="admin-login"><label>운영자 접근 키<input type="password" value={token} autoComplete="off" onChange={e => setToken(e.target.value)} required /></label><p>운영 담당자에게 전달받은 접근 키를 입력하세요.</p>{isLocal ? <p className="note">로컬 서버를 사용하는 경우 서버가 생성한 비밀 키 파일에서 접근 키를 확인할 수 있습니다.</p> : null}<button className="button" disabled={busy}>검토실 열기</button></form> : <>
    <div id="content" className="admin-grid">
     ...
     <section className="admin-edit">{creatingReview || selected?.reviewType === 'quote' ? <ReviewEditor key={`${selected?.id || 'new'}:${reviewSelection}:${selected?.revision || 0}`} item={creatingReview ? null : selected as QuoteReviewItem} api={api} onSaved={reviewSaved} onDirtyChange={setReviewDirty} onRefresh={load} /> : selected ? <>
      ...
      {selected.id.startsWith('shop-review-destination') ? <p className="note">공식몰 후기 원문으로 이동하는 고정 안내입니다. 문구는 수정하지 않고 공개·보류 상태만 관리합니다. 실제 인용 후기는 ‘새 인용 후기 등록’에서 원문과 게시 권한을 확인하여 등록하세요.</p> : null}
      <h3>근거 자료 · 읽기 전용</h3>
