...
export default function ProductShare({onEvent}:Props){
 ...
 return <aside className="product-share" aria-labelledby="product-share-heading"><div><h3 id="product-share-heading">함께 보고, 선택하세요.</h3><p>제품 구성이 궁금한 사람에게 구성 페이지를 보내세요.</p></div><div>
  <button type="button" className="button outline" disabled={busy} onClick={()=>void share()}>제품 구성 링크 공유 ↗</button>
