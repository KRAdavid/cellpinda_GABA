...
export default function OperationsMvp() {
  ...
  const reportLabel = report?.recommendation === 'approve'
    ...
    : report?.recommendation === 'blocked'
      ? '승인 대기'
      : report && report.completedTasks.length > 0 && report.pendingTasks.length === 0
        ? '샌드박스 완료 · 실제 검증 필요'
        : '추가 작업 필요';
