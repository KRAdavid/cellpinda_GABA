...
type DecisionQuorum = {minimum: number; roles: string[]; rule: string};
type QueueTask = {id: string; stream: string; title: string; state: string; lead: string; verifier: string; decision: string; decisionMode: string; nextAction: string; decisionOptions?: DecisionOption[]; quorum?: DecisionQuorum; risk?: string; blockedBy?: string; requiredInputs?: string[]; dependencies: string[]};
...
type ExecutionPolicy = {autoStates: string[]; autoRiskClasses: string[]; humanReviewStates: string[]; approvalRiskClasses: string[]; note: string};
type SafeExecutionSummary = {mode: string; status: string; validatedAt: string; executionBoundary: {state: string; risk: string; externalEffects: boolean}; preparation: {id: string; risk: string; status: string}; checks: {id: string; risk: string; status: string}[]; candidateTaskIds: string[]; humanGateTaskIds: string[]};
type QueueSnapshot = {schemaVersion: number; goalId: string; status: string; checkedAt: string; pulse?: {generatedAt: string; snapshotHash: string; stateChanged: boolean; requiresHumanDecision: boolean; continuation?: ContinuationLoop; meetingProtocol?: MeetingProtocol; executionPolicy?: ExecutionPolicy; safeExecution?: SafeExecutionSummary; activeTasks: number; inputGates: number}; roleCoverage?: RoleCoverage[]; workstreams: {id: string; name: string; lead: string; verifier: string; status: string; nextAction: string}[]; tasks: QueueTask[]};
...
export default function OperationsMvp() {
  ...
  const releaseHealth = <div className={`ops-mvp-release-health ops-mvp-release-health--${publicRelease.state}`} aria-live="polite" aria-label="공개 배포 상태">
