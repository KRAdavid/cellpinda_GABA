...
type QueueTask = {id: string; stream: string; title: string; state: string; lead: string; verifier: string; decision: string; decisionMode: string; nextAction: string; blockedBy?: string; dependencies: string[]};
type QueueSnapshot = {schemaVersion: number; goalId: string; status: string; checkedAt: string; pulse?: {generatedAt: string; snapshotHash: string; requiresHumanDecision: boolean; activeTasks: number; inputGates: number}; workstreams: {id: string; name: string; lead: string; verifier: string; status: string; nextAction: string}[]; tasks: QueueTask[]};
...
export default function OperationsMvp() {
  ...
  return <main className="ops-mvp">
    ...
    {queue ? <section className="ops-mvp-live-queue wrap" aria-labelledby="live-queue-heading">
      ...
      <div className="ops-mvp-live-queue-grid">{attentionQueue.map(task => <article key={task.id}><div><span className="ops-mvp-task-id">{task.id}</span><span className="ops-mvp-state">{stateLabels[task.state] || task.state}</span></div><h3>{task.title}</h3><p>담당 {task.lead} · 검증 {task.verifier}</p><small className="ops-mvp-live-decision">자동 판단 · {task.decision}</small><small>다음 조치 · {task.nextAction}</small>{task.blockedBy ? <small>대기 입력 · {task.blockedBy}</small> : null}</article>)}</div>
