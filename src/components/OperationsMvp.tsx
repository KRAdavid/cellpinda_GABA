...
type QueueTask = {id: string; stream: string; title: string; state: string; lead: string; verifier: string; decision: string; decisionMode: string; nextAction: string; decisionOptions?: DecisionOption[]; quorum?: DecisionQuorum; blockedBy?: string; requiredInputs?: string[]; dependencies: string[]};
...
export default function OperationsMvp() {
  ...
  return <main className="ops-mvp">
    ...
    {queue ? <section className="ops-mvp-live-queue wrap" aria-labelledby="live-queue-heading">
      ...
      <div className="ops-mvp-live-queue-grid" aria-label="업무별 현재 상태">{queue.tasks.map(task => <article key={task.id}><div><strong>{task.id}</strong><span>{stateLabels[task.state] || task.state}</span></div><h3>{task.title}</h3><p className="ops-mvp-live-decision">{task.decision}</p><small>담당 {task.lead} · 독립 검증 {task.verifier}</small>{task.quorum ? <small>결정 정족수 · {task.quorum.minimum}인 · {task.quorum.roles.join(' · ')}</small> : null}{task.decisionOptions?.length ? <small>회의 선택지 · {task.decisionOptions.map(option => option.label).join(' · ')}</small> : null}{task.requiredInputs?.length ? <small className="ops-mvp-required-inputs">필요 입력 · {task.requiredInputs.join(' · ')}</small> : null}{task.blockedBy ? <small>대기 이유 · {task.blockedBy}</small> : null}<small>다음 · {task.nextAction}</small></article>)}</div>
