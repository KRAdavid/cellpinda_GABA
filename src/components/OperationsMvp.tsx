...
export default function OperationsMvp() {
  const [input, setInput] = useState(defaultGoal);
  const [plan, setPlan] = useState<MvpPlan | null>(null);
  const [audit, setAudit] = useState<SandboxAuditEvent[]>([]);
  const [approval, setApproval] = useState<MvpApprovalReport['approval']>();
  ...
  function createPlan(event: React.FormEvent) {
    ...
    try {
      setPlan(generateMvpPlan(input)); setAudit([]); setApproval(undefined); setMessage('Goal Contract, TF, 업무 그래프를 생성했습니다.'); setCopied(false);
  ...
  function execute(taskId: string, token?: string) {
    ...
    try {
      ...
      if (result.approval) {
        setApproval(result.approval); setMessage('외부 약속 단계는 승인 전까지 실행하지 않습니다. 승인 보고서를 확인하세요.');
        return;
      }
      setPlan({...plan, tasks: result.tasks}); setAudit(items => [...items, ...result.events]); setApproval(undefined); setMessage(`${taskId} 작업을 검증 완료 상태로 기록했습니다.`);
