...
export default function OperationsMvp() {
  ...
  return <main className="ops-mvp">
    ...
    {queue ? <section className="ops-mvp-live-queue wrap" aria-labelledby="live-queue-heading">
      ...
      <p className="note">계약 확인일 {queue.checkedAt} · 완료 {queue.tasks.filter(task => task.state === 'DONE').length}건 · 진행/대기 {attentionQueue.length}건. 대기 입력이 도착하면 담당 TF가 검토 후 다음 작업을 엽니다.</p>{queue.pulse ? <p className="ops-mvp-live-pulse">마지막 TF pulse {formatTime(queue.pulse.generatedAt)} · 상태 지문 {queue.pulse.snapshotHash.slice(0, 12)}… · 사람 판단 {queue.pulse.requiresHumanDecision ? '필요' : '없음'} · 입력 게이트 {queue.pulse.inputGates}건</p> : null}{queue.pulse?.requiresHumanDecision ? <div className="ops-mvp-pulse-agenda" aria-labelledby="pulse-agenda-heading"><div><span className="ops-mvp-eyebrow" id="pulse-agenda-heading">다음 TF 회의 안건</span><p>자동 pulse가 멈춘 지점을 사람 회의에서 확인하고, 담당자와 검증자가 다음 조치를 합의합니다.</p></div><ol>{attentionQueue.filter(task => ['VERIFYING', 'WAITING'].includes(task.state)).slice(0, 5).map(task => <li key={task.id}><div><strong>{task.id}</strong><span>{task.decision}</span></div><small>참여 · {task.lead} · {task.verifier} · 다음 · {task.nextAction}</small></li>)}</ol></div> : null}
    </section> : null}
    {plan ? <>
