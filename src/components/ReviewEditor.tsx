...
type Props = { item: QuoteReviewItem | null; api: (path: string, options?: RequestInit) => Promise<unknown>; onSaved: (item: QuoteReviewItem) => void | Promise<void>; onDirtyChange: (dirty: boolean) => void; onRefresh: () => void | Promise<void> };
...
export default function ReviewEditor({ item, api, onSaved, onDirtyChange, onRefresh }: Props) {
  ...
  async function submit(action: 'save' | 'approved' | 'hold') {
    if (!reason.trim() || conflict || (action === 'approved' && !approvedReady) || (action !== 'save' && (!item || dirty))) return;
  ...
  return <div className="review-editor"><h2>{item ? '인용 후기 편집' : '새 후기 초안'}</h2><p className="note">인용문은 원문 직접 인용·발췌만 등록하세요. 요약 재작성은 지원하지 않으며 사용 맥락은 별도 설명입니다. 미완성 초안도 보류 저장할 수 있습니다.</p>
    ...
    <div className="review-editor-fields"><label htmlFor="quote-product">실제 사용 제품<select id="quote-product" value={review.productId} onChange={event => edit('productId', event.target.value)} disabled={busy}><option value="">미확인</option><option value="gaba750">셀핀다 가바 750</option><option value="gaba1500">셀핀다 가바 1500</option></select></label>
      {fields.map(field => <label key={field.key} htmlFor={`quote-${field.key}`}>{field.label}{field.private ? <small>내부 검토용</small> : null}{field.multiline ? <textarea id={`quote-${field.key}`} rows={3} maxLength={field.max} value={review[field.key]} onChange={event => edit(field.key, event.target.value)} disabled={busy} /> : <input id={`quote-${field.key}`} type={field.type || 'text'} maxLength={field.max} value={review[field.key]} onChange={event => edit(field.key, event.target.value)} disabled={busy} />}</label>)}
    </div>
    ...
    <button type="button" className="button outline" disabled={busy || conflict || !reason.trim()} onClick={() => void submit('save')}>{item ? '수정본 보류 저장' : '초안 보류 저장'}</button>
