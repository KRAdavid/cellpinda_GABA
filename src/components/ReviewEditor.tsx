...
export default function ReviewEditor({ item, api, onSaved, onDirtyChange, onRefresh }: Props) {
  ...
  return <div className="review-editor"><h2>{item ? '인용 후기 편집' : '새 후기 초안'}</h2><p className="note">인용문은 원문 직접 인용·발췌만 등록하세요. 요약 재작성은 지원하지 않으며 사용 맥락은 별도 설명입니다. 미완성 초안도 보류 저장할 수 있습니다.</p>
    ...
    <div className="review-editor-fields"><label htmlFor="quote-product">실제 사용 제품<select id="quote-product" value={review.productId} onChange={event => edit('productId', event.target.value)} disabled={busy}><option value="">미확인</option><option value="gaba1500">셀핀다 가바 1500</option></select></label>
