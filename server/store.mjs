...
const PUBLIC_METADATA_FIELDS = new Set(['studyType','population','sampleSize','dose','duration','comparison','outcome','productApplicability','question','searchThrough','studyCount','consumerScope','consumerSummary','consumerFinding','consumerHighlight','consumerFindingFirst','consumerVisual','consumerDetail','consumerContext','consumerDisclosure','hopefulTakeaway']);
...
function reconcileSeed(db, initial) {
  ...
  try {
    ...
    for (const [kind, seedMap] of Object.entries(seedByKind)) {
      for (const row of db.prepare('SELECT content.*, (SELECT reason FROM audit WHERE audit.content_id=content.id ORDER BY audit.id DESC LIMIT 1) AS last_reason FROM content WHERE kind=?').all(kind)) {
        ...
        } else if (kind === 'claim' && canonical.status === 'hold') {
          if(before.status!=='hold' || before.holdReason!==canonical.holdReason) {
            after = {...before,status:'hold',holdReason:canonical.holdReason || '현재 출처 검토에 따라 공개 보류'};
            reason = 'Current source ledger safety hold; operator copy preserved';
          }
...
export function createStore({ dbPath, seedPath, seed } = {}) {
  ...
  const sourceSeedText = seed === undefined ? readFileSync(seedPath, 'utf8') : undefined;
  const initial = seed ?? JSON.parse(sourceSeedText);
  try { reconcileSeed(db, initial); } catch (error) { db.close(); throw error; }
  let lastSeedHash = sourceSeedText === undefined ? undefined : createHash('sha256').update(sourceSeedText,'utf8').digest('hex');
  const syncSourceSeed = () => {
    if (seed !== undefined || !seedPath) return false;
    const sourceText=readFileSync(seedPath,'utf8');
    const hash=createHash('sha256').update(sourceText,'utf8').digest('hex');
    if(hash===lastSeedHash)return false;
    const latest=JSON.parse(sourceText);
    reconcileSeed(db,latest);
    lastSeedHash=hash;
    return true;
  };
